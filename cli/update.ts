...
function resolveInstallDir(): string {
  return process.env.BEAM_INSTALL_DIR ?? join(homedir(), ".beam");
}

function resolveWrapperBin(): string {
  const fromEnv = process.env.BEAM_WRAPPER_BIN;
  if (fromEnv) return fromEnv;
  const fromPath = Bun.which("beam");
  if (fromPath) return fromPath;
  return join(homedir(), ".local", "bin", "beam");
}
...
function atomicReplaceInstall(installDir: string, newDir: string): string {
  ...
  const backupDir = join(parent, `.beam-update-backup-${Date.now()}`);
...
function persistPreviousInstall(
  ...
  const staleBackups = readdirSync(parent)
    .filter((name) => {
      ...
      return name.startsWith(`${base}.bak-`) || name.startsWith(".beam-update-backup-");
    })
...
function rewriteBundleWrapper(wrapperBin: string, installDir: string): void {
  ...
  try {
    ...
    if (!existing.includes("Auto-generated by Beam installer")) return;
    const extension = process.platform === "darwin" ? "dylib" : "so";
    const content = `#!/usr/bin/env bash
# Auto-generated by Beam installer. Runs the compiled bundle.
export BEAM_INSTALL_DIR="${installDir}"
export BEAM_VEC0_PATH="${installDir}/lib/vec0.${extension}"
...
function findSourceRoot(extractedDir: string): string {
  ...
  throw new UpdateAbort("Extracted source tarball does not contain a valid Beam source tree");
}

function preserveUserData(sourceDir: string, destDir: string): void {
  const items = [".env", "beam.db", "beam.db-wal", "beam.db-shm", "logs"];
...
function migrateBundleStateToUserDirs(installDir: string): void {
  const configDir = process.env.BEAM_CONFIG_DIR ?? join(homedir(), ".config", "beam");
  const dataDir = process.env.BEAM_DATA_DIR ?? join(homedir(), ".local", "share", "beam");
  ...
  for (const item of ["beam.db", "beam.db-wal", "beam.db-shm", "logs"]) {
...
async function updateFromSource(
  ...
  const bundleName = `beam-v${release.version}.tar.gz`;
  ...
  console.log("Preserving user data (.env, beam.db, logs)...");
...
export const updateCommand = new Command("update")
  ...
  .action(async (options) => {
    ...
    try {
      ...
      workDir = mkdtempSync(join(tmpdir(), "beam-update-"));
