...
import { buildRewardClaimMetadata, summarizeRewardClaim, type ClaimedReward } from "./rewards.js";
import { errorReporter } from "./error-reporter.js";
import { BlacklistError, ConfigError, DiscoverPoolsError, underlyingErrorMessage } from "./errors.js";
import {
...
import { detectDepegAndLiquidityDrain, type DepegLiquiditySignals } from "./depeg-liquidity-detector.js";
...
function isTrackedPositionGone(
  positionPubKey: string | null,
  liveByPubkey: ReadonlyMap<string, unknown>,
): boolean {
  return !!positionPubKey && !liveByPubkey.has(positionPubKey);
}

function owedAmounts(
  onChain: { readonly tokensOwedX?: bigint; readonly tokensOwedY?: bigint } | undefined,
): { owedX: bigint; owedY: bigint } {
  return { owedX: onChain?.tokensOwedX ?? 0n, owedY: onChain?.tokensOwedY ?? 0n };
}

function hasUnclaimedOwedFees(owedX: bigint, owedY: bigint): boolean {
  return owedX > 0n || owedY > 0n;
}

function isRangeDrifted(
  trackedLower: number,
  trackedUpper: number,
  onChainLower: number,
  onChainUpper: number,
): boolean {
  return trackedLower !== onChainLower || trackedUpper !== onChainUpper;
}

function isOrphanPool(
  poolAddress: string,
  watchedPoolSet: ReadonlySet<string>,
  adoptedPoolAddresses: ReadonlySet<string>,
): boolean {
  return !watchedPoolSet.has(poolAddress) && !adoptedPoolAddresses.has(poolAddress);
}
...
export function reconcilePositions(
  ...
  return Effect.gen(function* () {
    ...
    for (const [positionId, pos] of trackedPositions) {
      if (isTrackedPositionGone(pos.positionPubKey, liveByPubkey)) {
        ...
        const { owedX, owedY } = owedAmounts(onChainByPubkey.get(pos.positionPubKey!));
        if (hasUnclaimedOwedFees(owedX, owedY)) {
          console.warn(
    ...
    for (const onChainPos of liveOnChain) {
      ...
      if (tracked) {
        ...
        if (
          isRangeDrifted(
            tracked.lowerBinId,
            tracked.upperBinId,
            onChainPos.lowerBinId,
            onChainPos.upperBinId,
          )
        ) {
          console.warn(
      ...
      if (isOrphanPool(onChainPos.poolAddress, watchedPoolSet, adoptedPoolAddresses)) {
...
function buildAgentLayer(cfg?: AppConfig) {
  if (cfg === undefined || !cfg.agentiveMode) return Layer.succeed(AgentService, AgentNoOp);
  return AgentLive(cfg);
}

function buildMcpLayer(
  cfg: AppConfig | undefined,
  state: ReturnType<typeof AgentStateMutable>["layer"],
) {
  if (cfg === undefined || !cfg.agentMcpEnabled) {
    return Layer.succeed(McpServerService, { start: () => Effect.void, stop: () => Effect.void });
  }
  return Layer.provide(McpServerLive(cfg), state);
}

function buildHttpLayer(
  cfg: AppConfig | undefined,
  state: ReturnType<typeof AgentStateMutable>["layer"],
) {
  if (cfg === undefined || cfg.agentHttpPort <= 0) {
    return Layer.succeed(HttpStatusServerService, {
      start: () => Effect.void,
      stop: () => Effect.void,
    });
  }
  return Layer.provide(HttpStatusServerLive(cfg), state);
}

export function buildLayer(
  cfg?: AppConfig,
): Layer.Layer<AllServices, ConfigError | Config.ConfigError, never> {
  ...
  const screener = Layer.provide(
    ScreenerLive({
      minTvlUsd: withDefault(cfg?.discoveryMinTvlUsd, 1_000_000),
      minFeeRatio: withDefault(cfg?.discoveryMinFeeRatio, 1.5),
      volumeAuthThreshold: withDefault(cfg?.volumeAuthThreshold, 0.7),
      minBinUtilization: withDefault(cfg?.minBinUtilization, 0.3),
    }),
    screenerDeps,
  );
  const risk = RiskLive({
    confidenceThreshold: withDefault(cfg?.confidenceThreshold, 0.65),
    maxRebalanceRangeBins: withDefault(cfg?.maxRebalanceRangeBins, 50),
    stopLossPct: withDefault(cfg?.stopLossPct, 0.15),
    maxPerPoolAllocationPct: withDefault(cfg?.maxPerPoolAllocationPct, 0.4),
    maxPositionsPerPool: withDefault(cfg?.maxPositionsPerPool, 2),
    maxObservedPriceRangePct: withDefault(cfg?.maxObservedPriceRangePct, 30),
  });
  const blacklist = BlacklistLive({
    deployerBlacklistPath: withDefault(
      cfg?.deployerBlacklistPath,
      "./engine/data/deployer-blacklist.json",
    ),
    tokenBlacklistPath: withDefault(cfg?.tokenBlacklistPath, "./engine/data/token-blacklist.json"),
  });
  ...
  const agentLayer = buildAgentLayer(cfg);
  const agentStateLayer = AgentStateMutable({
    maxPendingProposals: withDefault(cfg?.agentProposalMaxQueueSize, 50),
  }).layer;
  const mcpLayer = buildMcpLayer(cfg, agentStateLayer);
  const httpLayer = buildHttpLayer(cfg, agentStateLayer);
  ...
  const out: Layer.Layer<AllServices, ConfigError | Config.ConfigError, never> = merged17;
  return out;
}

// ─── Paper execution ─────────────────────────────────────────────────────────

export interface ExecutePaperDeps {
  readonly db: DbApi;
  readonly trackedPositions: Map<string, PositionRecord>;
  readonly strategy: StrategyApi;
  readonly entryStrategyMode: EntryStrategyMode;
  readonly entryRangeHalfWidth?: number;
  readonly paperGasCostUsd?: number;
}

export interface ExecutePaperPool {
  readonly activeBinId: number;
...
function executePaperEnter(
  deps: ExecutePaperDeps,
  decision: AgentDecision,
  pool: ExecutePaperPool,
  ...
  return Effect.gen(function* () {
    ...
    const pos: PositionRecord = {
      ...
      entryAmountXUsd: positionSizeUsd / 2,
      entryAmountYUsd: positionSizeUsd / 2,
      cumulativeFeesClaimedUsd: 0,
      cumulativeRewardsClaimedUsd: 0,
      closedAt: null,
      realizedPnlUsd: null,
...
function executePaperRebalance(
  ...
  return Effect.gen(function* () {
    const { db, trackedPositions } = deps;
    if (decision.rebalanceParams === undefined) return;
...
export function executePaper(
  ...
  return Effect.gen(function* () {
    if (decision.action === "ENTER" && decision.positionSizeUsd) {
      ...
      yield* executePaperRebalance(deps, decision, pool);
    }
    return { executed: true, error: undefined };
  });
}
 ...
 */
export interface ExecuteLiveDeps {
  readonly adapter: AdapterApi;
  readonly strategy: StrategyApi;
  readonly db: DbApi;
  readonly revenueConfigSvc: RevenueConfigApi;
  readonly trackedPositions: Map<string, PositionRecord>;
  readonly nativePriceUsd: number;
  readonly entryStrategyMode: EntryStrategyMode;
  readonly entryRangeHalfWidth?: number;
  readonly minNativeForEntryWei?: bigint | undefined;
  readonly minNativeForGasWei?: bigint | undefined;
  readonly gasReservePct?: number | undefined;
  readonly entryMinRangePct?: number | undefined;
  readonly reconcileRequestedPools?: Set<string>;
  readonly memory?: MemoryApi;
  readonly unpricedExitWarnedPools?: Set<string>;
  readonly autonomous?: AutonomousExecutionContext;
  readonly candidateId?: string;
}

export interface ExecuteLivePool {
  readonly activeBinId: number;
  readonly binStep: number;
  readonly tokenXSymbol: string;
  readonly tokenYSymbol: string;
...
function updateExecutionOperation(
  db: DbApi,
...
function planLiveEntryOperation(
  autonomous: AutonomousExecutionContext | undefined,
  db: DbApi,
  decision: AgentDecision,
  pool: ExecuteLivePool,
): Effect.Effect<{ error: string } | { operation: ExecutionOperationRecord | null }, never, never> {
  return Effect.gen(function* () {
    if (!(decision.action === "ENTER" && decision.positionSizeUsd && autonomous && pool.tokenX)) {
      return { operation: null };
    }
    const now = Date.now();
    const entryOperation = operationRecord({
      context: autonomous,
      id: randomUUID(),
      candidateId: null,
      positionId: null,
    ...
    if (!persisted) {
      return { error: "Unable to persist entry operation before execution" };
    }
    return { operation: entryOperation };
  });
}

function topUpNativeForEntry(
  adapter: AdapterApi,
  ...
  return Effect.gen(function* () {
    ...
    // stablecoin leg).
    const entryReserveWei = minNativeForEntryWei ?? NATIVE_GAS_TOP_UP_THRESHOLD_WEI;
...
function readNativeBalanceGate(
  adapter: AdapterApi,
  db: DbApi,
  autonomous: AutonomousExecutionContext | undefined,
...
): Effect.Effect<{ ok: true; wei: bigint } | { ok: false; error: string }, never, never> {
  return Effect.gen(function* () {
    ...
    if (nativeBalance.value === null) {
      ...
      if (autonomous && entryOperation) {
        yield* failPlannedEntryOp(db, entryOperation, error);
      }
      return { ok: false as const, error };
    }
    const nativeBalanceWei = nativeBalance.value;
    const minEntryWei = minNativeForEntryWei ?? MIN_NATIVE_FOR_ENTRY_WEI;
    const minGasWei = minNativeForGasWei ?? MIN_NATIVE_FOR_GAS_WEI;
    ...
    const entryBudget = entryBudgetWei(nativeBalanceWei, {
      reservePct: Math.round((gasReservePct ?? 0.1) * 100),
      emergencyExitWei: minGasWei,
    });
    if (entryBudget < minEntryWei) {
      const availableWei = Number(nativeBalanceWei);
      const neededWei = Number(minEntryWei);
      ...
      console.warn(
        `Insufficient ETH for ENTER — spendable ${availableHuman} ETH, need ${neededHuman} ETH ` +
          `(gas ${(Number(minGasWei) / 1e18).toFixed(6)} + reserve ${reserveHuman})`,
      );
      const error =
    ...
    return { ok: true as const, wei: nativeBalanceWei };
  });
}

function executeLiveEnterMint(
  deps: ExecuteLiveDeps,
  decision: AgentDecision,
  pool: ExecuteLivePool,
  ...
  const { adapter, strategy, db, trackedPositions, entryStrategyMode, entryRangeHalfWidth } = deps;
  return Effect.gen(function* () {
    if (positionSizeUsd === undefined) {
      const error = "ENTER decision missing position size";
      if (entryOperation) {
        yield* updateExecutionOperation(db, entryOperation, {
          status: "failed",
          error,
          txSignature: null,
        });
...
function planLiveExitOperation(
  ...
  return Effect.gen(function* () {
    ...
    if (!persisted) {
      return { error: "Unable to persist exit operation before execution" };
    }
    return { operation: exitOperation };
  });
}

type LiveExitAttempt = {
  readonly exited: boolean;
  readonly exitError: string | undefined;
  readonly exitResultData: Effect.Success<ReturnType<AdapterApi["exitPosition"]>> | null;
  readonly exitOperation: ExecutionOperationRecord | null;
};

function probeMinedExit(
  adapter: AdapterApi,
  poolAddress: string,
  positionPubKey: string,
): Effect.Effect<Effect.Success<ReturnType<AdapterApi["exitPosition"]>> | null, never, never> {
  return Effect.gen(function* () {
    ...
    const stillOpen = livePositions.some((p) => p.positionPubKey === positionPubKey);
...
function runLiveExitAttempt(
  ...
  return Effect.gen(function* () {
    ...
    if (pos?.positionPubKey) {
      ...
      const exitGasFloorWei = minNativeForGasWei ?? MIN_NATIVE_FOR_GAS_WEI;
      ...
      if (nativeBeforeExit !== null && nativeBeforeExit < exitGasFloorWei) {
        ...
        if (operation) {
          yield* updateExecutionOperation(db, operation, {
            status: "failed",
            error,
            txSignature: null,
          });
        }
        return { exited, exitError: error, exitResultData, exitOperation: operation };
      }
      ...
      if (operation && exitResult.result) {
        operation = {
          ...operation,
          status: "confirmed",
        ...
        yield* db.saveExecutionOperation(operation).pipe(Effect.catch(() => Effect.void));
      }
      if (!exited) {
        // A receipt-wait timeout can fire AFTER the tx mined: the EXIT
        // actually landed but the adapter gave up waiting. Probe the chain —
        ...
        // PnL, honest) instead of reconcile silently deleting the row and
        // losing the trade from the ledger + leaving peak equity stale.
        const minedData = yield* probeMinedExit(adapter, decision.poolAddress, pos.positionPubKey);
        if (minedData) {
          exited = true;
          exitResultData = minedData;
        }
        if (!exited) {
          ...
          reconcileRequestedPools?.add(decision.poolAddress);
        }
      }
    } else {
      exited = true;
    }
    return { exited, exitError, exitResultData, exitOperation: operation };
  });
}

function syncExitOperationState(
  db: DbApi,
  exitOperation: ExecutionOperationRecord | null,
  exited: boolean,
  exitError: string | undefined,
  exitResultData: Effect.Success<ReturnType<AdapterApi["exitPosition"]>> | null,
): Effect.Effect<void, never, never> {
  return Effect.gen(function* () {
    if (exitOperation?.status !== "planned") return;
    if (exited) {
      yield* updateExecutionOperation(db, exitOperation, {
        status: "confirmed",
        error: null,
...
function recordExitSettlements(
  db: DbApi,
  ...
  tokenX: string,
  tokenY: string,
  exitResultData: Effect.Success<ReturnType<AdapterApi["exitPosition"]>>,
): Effect.Effect<
  { fatal: string } | { ok: true; now: number; attributableCount: number },
  never,
  never
> {
  return Effect.gen(function* () {
    ...
    return { ok: true as const, now, attributableCount: attributable.length };
  });
}
...
function closeExitedPosition(
  ...
  return Effect.gen(function* () {
    ...
    const realizedPnlUsd =
      pricingUnresolved || attributableCount > 0
        ? null
        : computeRealizedPnlUsd(
            withdrawnUsd,
            pos.cumulativeFeesClaimedUsd,
    ...
    yield* db
      .savePositionEvent({
        ...
        price: pool.currentPrice,
        metadata: {
          settlementPending: attributableCount,
          txSignature: exitResultData.txSignature,
        },
        createdAt: Date.now(),
      })
...
function creditSweptRewards(
  ...
  decision: AgentDecision,
  pool: ExecuteLivePool,
  sweptRewards: ReadonlyArray<ClaimedReward>,
): Effect.Effect<number, never, never> {
  return Effect.gen(function* () {
    let pricedSweptRewardUsd = 0;
    ...
    for (const reward of sweptRewards) {
      if (typeof reward.amountUsd === "number") {
        ...
        pricedSweptRewardUsd += reward.amountUsd;
      } else {
        unpricedReward = true;
      }
    }
    ...
    if (sweptRewards.length > 0) {
      yield* db
        .savePositionEvent({
          ...
          positionId: pos.positionId,
          event: "CLAIM",
          valueUsd: pricedSweptRewardUsd > 0 ? pricedSweptRewardUsd : null,
          feesUsd: null,
          price: pool.currentPrice,
          metadata: {
...
function warnUnpricedExit(
  memory: MemoryApi | undefined,
  unpricedExitWarnedPools: Set<string> | undefined,
  pos: PositionRecord,
  poolAddress: string,
): Effect.Effect<void, never, never> {
  return Effect.gen(function* () {
    unpricedExitWarnedPools?.add(poolAddress);
    logger.warn(
      // SAFETY: this assertion is applied to a value whose producer and invariant are controlled by this code path.
      "EXIT closed without USD pricing (price feeds unresolved) — realized PnL recorded as n/a; raw amounts in event metadata",
      { pool: poolAddress, position: pos.positionId },
    );
    yield* memory
      ? memory
          .upsert({
            category: "warning",
            // SAFETY: this assertion is applied to a value whose producer and invariant are controlled by this code path.
            content: `EXIT on ${poolAddress} closed without USD pricing (price feeds unresolved) — realized PnL recorded as n/a; raw amounts in event metadata`,
            poolAddress,
          })
          .pipe(Effect.catch(() => Effect.void))
      : Effect.void;
...
function creditExitSweep(
  ...
  return Effect.gen(function* () {
    ...
    yield* creditSweptRewards(db, pos, decision, pool, exitResultData?.sweptRewards ?? []);
  });
}

function settleUnresolvedExit(
  db: DbApi,
  trackedPositions: Map<string, PositionRecord>,
  memory: MemoryApi | undefined,
  unpricedExitWarnedPools: Set<string> | undefined,
  ...
  return Effect.gen(function* () {
    ...
    yield* creditExitSweep(db, pos, decision, pool, exitResultData);
    // 4. Persist the credited cumulatives onto the (still-open) row, THEN
    ...
    const exitMetadata = buildExitMetadata(pos, exitResultData, realizedPnlUsd, pricingUnresolved);
    yield* db
      .savePositionEvent({
        id: randomUUID(),
        poolAddress: decision.poolAddress,
      ...
      .pipe(Effect.catch(() => Effect.void));
    if (pricingUnresolved && !(unpricedExitWarnedPools?.has(decision.poolAddress) ?? false)) {
      yield* warnUnpricedExit(memory, unpricedExitWarnedPools, pos, decision.poolAddress);
    }
    trackedPositions.delete(pos.positionId);
    return { executed: true, error: undefined };
  });
...
function claimFeesForRebalance(
  db: DbApi,
  adapter: AdapterApi,
  ...
  pos: PositionRecord,
  positionPubKey: string,
): Effect.Effect<{ claimed: ClaimFeesResult; tier: string } | null, never, never> {
  return Effect.gen(function* () {
    ...
    if (claimResult && (claimResult.feeX > 0 || claimResult.feeY > 0)) {
      ...
      return { claimed: claimResult, tier: revenueConfigResult.tier };
    }
    return null;
  });
}
...
): Effect.Effect<{ ok: true } | { ok: false; error: string }, never, never> {
  return Effect.gen(function* () {
    ...
    return { ok: false as const, error: rebalanceResult.error ?? "Atomic rebalance failed" };
  });
}

type ClaimFeesResult = Effect.Success<ReturnType<AdapterApi["claimFees"]>>;
type ClaimRewardsResult = Effect.Success<ReturnType<AdapterApi["claimRewards"]>>;
type PendingFeesResult = Effect.Success<ReturnType<NonNullable<AdapterApi["getPendingFees"]>>>;

function reportCollectedFees(
  adapter: AdapterApi,
  poolAddress: string,
  positionPubKey: string | null,
  claimed: ClaimFeesResult,
  tier: string,
): Effect.Effect<void, never, never> {
  return Effect.gen(function* () {
    if (
      claimed.platformFeeX > 0 ||
      claimed.platformFeeY > 0 ||
      (claimed.operatorFeeX ?? 0) > 0 ||
      (claimed.operatorFeeY ?? 0) > 0
    ) {
      yield* Effect.forkChild(
        adapter
          .reportFeeCollection({
            ...
            feeX: claimed.feeX,
            feeY: claimed.feeY,
            platformFeeX: claimed.platformFeeX,
            platformFeeY: claimed.platformFeeY,
            tier,
            txSignature: claimed.txSignature,
            ...(claimed.feeTransferTxSignature != null && {
              feeTransferTxSignature: claimed.feeTransferTxSignature,
            }),
            ...(claimed.operatorFeeX != null && {
              operatorFeeX: claimed.operatorFeeX,
            }),
            ...(claimed.operatorFeeY != null && {
              operatorFeeY: claimed.operatorFeeY,
            }),
          })
          .pipe(
            Effect.catchCause((cause) =>
...
function hasExitSettlementContext(
  autonomous: AutonomousExecutionContext | undefined,
  pos: PositionRecord | undefined,
  exitResultData: Effect.Success<ReturnType<AdapterApi["exitPosition"]>> | null,
...
function settleAttemptedExit(
  ...
  return Effect.gen(function* () {
    if (!hasExitSettlementContext(autonomous, pos, exitResultData, pool.tokenX, pool.tokenY)) {
      if (pos) {
        return yield* settleUnresolvedExit(
          db,
          trackedPositions,
          memory,
    ...
    const settled = yield* recordExitSettlements(
      db,
      autonomous!,
      pos!,
      decision,
    ...
    if ("fatal" in settled) return { executed: false, error: settled.fatal };
    yield* recordExitSweepEvents(db, pos!, decision, pool, exitResultData!, settled.now);
    yield* closeExitedPosition(
      db,
      trackedPositions,
      pos!,
      decision,
      pool,
      exitResultData!,
      settled.attributableCount,
    );
    return { executed: true, error: undefined };
  });
}

function flagPoolForReconcile(
  reconcileRequestedPools: Set<string> | undefined,
  poolAddress: string,
): void {
  reconcileRequestedPools?.add(poolAddress);
}
...
export function executeLive(
  ...
  return Effect.gen(function* () {
    const { adapter, db, trackedPositions, revenueConfigSvc } = deps;
    ...
    const plannedEntry = yield* planLiveEntryOperation(autonomous, db, decision, pool);
    if ("error" in plannedEntry) return { executed: false, error: plannedEntry.error };
    const entryOperation = plannedEntry.operation;
    if (decision.action === "ENTER") {
      yield* topUpNativeForEntry(adapter, deps.minNativeForEntryWei, deps.nativePriceUsd);
      const gated = yield* readNativeBalanceGate(
        adapter,
        db,
        autonomous,
        entryOperation,
        deps.minNativeForEntryWei,
        deps.minNativeForGasWei,
        deps.gasReservePct,
      );
      if (!gated.ok) return { executed: false, error: gated.error };
      return yield* executeLiveEnterMint(
        deps,
        decision,
        pool,
        decision.positionSizeUsd,
    ...
    if (decision.action === "EXIT") {
      const pos = resolveTargetPosition(trackedPositions, decision);
      const exitPlan = yield* planLiveExitOperation(
        decision,
        pool,
        pos,
        autonomous,
      ...
      if ("error" in exitPlan) return { executed: false, error: exitPlan.error };
      const attempted = yield* runLiveExitAttempt(
        adapter,
        db,
        decision,
        pos,
        exitPlan.operation,
        deps.minNativeForGasWei,
        deps.reconcileRequestedPools,
      );
      yield* syncExitOperationState(
        db,
        attempted.exitOperation,
        attempted.exited,
        attempted.exitError,
        attempted.exitResultData,
      );
      if (attempted.exited) {
        return yield* settleAttemptedExit(
          db,
          trackedPositions,
          autonomous,
          pos,
          ...
          attempted.exitResultData,
          deps.memory,
          deps.unpricedExitWarnedPools,
        );
      }
      return { executed: false, error: attempted.exitError };
    }
    if (decision.action === "REBALANCE" && decision.rebalanceParams) {
      const rpos = resolveTargetPosition(trackedPositions, decision);
      if (rpos?.positionPubKey) {
        yield* claimFeesForRebalance(
          db,
          adapter,
          revenueConfigSvc,
          decision,
        ...
        const rebalanced = yield* executeAtomicRebalance(
          db,
          trackedPositions,
          adapter,
          decision.poolAddress,
        ...
        if (rebalanced.ok) return { executed: true, error: undefined };
        logger.warn("Atomic rebalance failed — flagging pool for reconcile", {
          pool: decision.poolAddress,
          error: rebalanced.error,
        });
        flagPoolForReconcile(deps.reconcileRequestedPools, decision.poolAddress);
        return { executed: false, error: rebalanced.error };
      }
...
function buildMarketGateConfig(config: AppConfig): MarketGateConfig {
  return {
...
function marketScanActiveCount(config: AppConfig): number {
  return Math.min(config.marketScanTopK ?? 30, config.marketScanMaxPools ?? 60);
}

function formatTopMarketRanks(ranked: ReadonlyArray<MarketPoolRank>): string {
  return ranked
    .slice(0, 5)
    .map(
      (r) =>
...
// ─── Per-position EXIT decision builders ─────────────────────────────────────
// Pure functions returning decision descriptors for runExitPhase. The generator
// loop only iterates, sends the precomputed alerts, and pushes decisions.

/** IL-dominance measurements feeding the IL-dominance EXIT alert/decision. */
type ExitIlDominanceDetail = {
  readonly ilUsd: number;
  readonly hodlValueUsd: number;
  readonly feesClaimedUsd: number;
};

/** Side-effect payloads a winning EXIT branch precomputes for the loop. */
type PositionExitBranchOutcome = {
  ...
  readonly agentAlert: {
    readonly severity: AgentRuntimeAlert["severity"];
    readonly category: AgentRuntimeAlert["category"];
    readonly message: string;
  } | null;
  readonly memoryWarning: string | null;
};

const noExitSideEffects: Pick<
  PositionExitBranchOutcome,
  "ilAlert" | "agentAlert" | "memoryWarning"
> = { ilAlert: null, agentAlert: null, memoryWarning: null };

function buildChallengeRotationExit(
  challengeRotation: { readonly action: string; readonly reason: string } | null,
  poolAddress: string,
  positionId: string,
): AgentDecision | null {
  if (challengeRotation === null) return null;
  if (challengeRotation.action === "hold") return null;
  return {
    action: "EXIT",
    poolAddress,
    positionId,
...
function evaluateExitIlDominance(input: {
  readonly ilProtectionEnabled: boolean;
  readonly outOfRangeSince: number | null;
  readonly entryAmountXUsd: number | null | undefined;
  readonly entryAmountYUsd: number | null | undefined;
  readonly entryPriceUsd: number | null | undefined;
  readonly currentValueUsd: number;
  readonly cumulativeFeesClaimedUsd: number;
  readonly currentPrice: number;
  readonly dominanceFactor: number | undefined;
  readonly minUsd: number | undefined;
}): ExitIlDominanceDetail | null {
  if (
    input.ilProtectionEnabled !== true ||
    input.outOfRangeSince === null ||
    input.entryAmountXUsd == null ||
    input.entryAmountYUsd == null ||
  ...
  if (hodlValueUsd === null) return null;
  const ilUsd = hodlValueUsd - input.currentValueUsd;
  const feesClaimedUsd = input.cumulativeFeesClaimedUsd;
  const factor = input.dominanceFactor ?? 2;
  const floor = input.minUsd ?? 5;
  if (ilUsd > 0 && ilUsd > feesClaimedUsd * factor && ilUsd > floor) {
    return { ilUsd, hodlValueUsd, feesClaimedUsd };
  }
  return null;
}

function evaluateExitFallenAngel(input: {
  readonly fallenAngelEnabled: boolean;
  readonly positionMode: string | null | undefined;
  readonly tpLadderJson: string | null | undefined;
  readonly invalidationStopPrice: number | null | undefined;
  readonly currentPrice: number;
}): { readonly status: "tp" | "invalidation"; readonly reasoning: string } | null {
  if (input.fallenAngelEnabled !== true) return null;
  if (input.positionMode !== "fallen-angel") return null;
  const ladder = parseTpLadder(input.tpLadderJson);
  if (ladder === null || input.invalidationStopPrice == null) return null;
  const evaluation = evaluateTpLadder(input.currentPrice, ladder, input.invalidationStopPrice);
  if (evaluation.status === "invalidation") {
    return {
      status: "invalidation",
      reasoning: `[fa-invalidation] Price ${input.currentPrice.toFixed(6)} <= invalidation stop ${input.invalidationStopPrice.toFixed(6)} — thesis broken`,
    };
  }
  if (evaluation.status === "tp" && evaluation.rungReached) {
    return {
      status: "tp",
      reasoning: `[fa-tp-ladder] Price ${input.currentPrice.toFixed(6)} reached target ${evaluation.rungReached.targetPrice.toFixed(6)} — scale out ${(evaluation.scaleOutFraction ?? 0) * 100}%`,
    };
  }
  return null;
}

function buildW15ExitOutcome(
  signals: DepegLiquiditySignals,
  poolAddress: string,
  positionId: string,
): PositionExitBranchOutcome | null {
  if (signals.depeg === null && signals.liquidityDrain === null) return null;
  const parts: string[] = [];
  if (signals.depeg !== null) parts.push("stablecoin depeg");
  if (signals.liquidityDrain !== null) parts.push("liquidity drain");
  return {
    decision: {
      action: "EXIT",
      poolAddress,
      positionId,
      confidence: 1,
      reasoning: `W15 fast EXIT: ${parts.join(" + ")}`,
    },
    ...noExitSideEffects,
  };
}

function buildIlDominanceOutcome(input: {
  readonly ilProtectionEnabled: boolean;
  readonly outOfRangeSince: number | null;
  readonly entryAmountXUsd: number | null | undefined;
  readonly entryAmountYUsd: number | null | undefined;
  ...
  const dominance = evaluateExitIlDominance(input);
  ...
  return {
    decision: {
      ...
      positionId: input.positionId,
      confidence: 1,
      reasoning: `IL dominance: $${dominance.ilUsd.toFixed(2)} IL exceeds ${factor}× cumulative fees ($${dominance.feesClaimedUsd.toFixed(2)}) while out of range`,
    },
    ilAlert: {
      message: `IL dominance EXIT on ${input.tokenXSymbol}/${input.tokenYSymbol} position ${input.positionId}: IL $${dominance.ilUsd.toFixed(2)} exceeds ${factor}× fees ($${dominance.feesClaimedUsd.toFixed(2)})`,
      ilUsd: dominance.ilUsd,
      hodlValueUsd: dominance.hodlValueUsd,
      feesClaimedUsd: dominance.feesClaimedUsd,
    },
    agentAlert: null,
    memoryWarning: null,
  };
...
function buildPoolDegradationOutcome(input: {
  readonly dustExitUsd: number | undefined;
  readonly currentValueUsd: number;
  ...
  const dustThreshold = input.dustExitUsd ?? 0;
  if (dustThreshold > 0 && input.currentValueUsd < dustThreshold) {
    return {
      decision: {
        action: "EXIT",
        poolAddress: input.poolAddress,
        positionId: input.positionId,
        confidence: 1,
        reasoning: `[dust-cleanup] Position value $${input.currentValueUsd.toFixed(2)} below $${dustThreshold.toFixed(2)} dust threshold — reclaiming slot`,
      },
      ilAlert: null,
      agentAlert: {
        severity: "warning",
        category: "risk_rejected",
        message: `Dust position closed on ${input.tokenXSymbol}/${input.tokenYSymbol}: value $${input.currentValueUsd.toFixed(2)} below $${dustThreshold.toFixed(2)}`,
      },
      memoryWarning: null,
    };
  }
  if (input.tvlVelocity < -input.tvlDropExitPct) {
    const dropPct = (Math.abs(input.tvlVelocity) * 100).toFixed(1);
    return {
      decision: {
        action: "EXIT",
        poolAddress: input.poolAddress,
        positionId: input.positionId,
        confidence: 0.85,
        reasoning: `TVL dropped ${dropPct}% — capital protection exit`,
      },
      ilAlert: null,
      agentAlert: {
        severity: "critical",
        category: "tvl_drop",
        message: `TVL dropped ${dropPct}% on ${input.tokenXSymbol}/${input.tokenYSymbol} — capital protection EXIT triggered`,
      },
      memoryWarning: `Pool ${input.poolAddress} TVL dropped sharply. Exit triggered.`,
    };
  }
  if (input.volumeAuthenticityKnown && input.volumeAuthenticity < input.volumeAuthThreshold) {
    const auth = input.volumeAuthenticity.toFixed(2);
    return {
      decision: {
        action: "EXIT",
        poolAddress: input.poolAddress,
        positionId: input.positionId,
        confidence: 0.8,
        reasoning: `Volume authenticity ${auth} below threshold`,
      },
      ilAlert: null,
      agentAlert: {
        severity: "warning",
        category: "risk_rejected",
        message: `Volume authenticity ${auth} below threshold on ${input.tokenXSymbol}/${input.tokenYSymbol} — EXIT`,
      },
      memoryWarning: null,
    };
  }
  if (input.feeIlRatioKnown && input.feeIlRatio < 0.5) {
    const ratio = input.feeIlRatio.toFixed(2);
    return {
      decision: {
        action: "EXIT",
        poolAddress: input.poolAddress,
        positionId: input.positionId,
        confidence: 0.75,
        reasoning: `Fee/IL ratio ${ratio} below 0.5`,
      },
      ilAlert: null,
      agentAlert: {
        severity: "warning",
        category: "risk_rejected",
        message: `Fee/IL ratio ${ratio} below 0.5 on ${input.tokenXSymbol}/${input.tokenYSymbol} — EXIT`,
      },
      memoryWarning: null,
    };
  }
...
function describeW15Alert(signals: DepegLiquiditySignals): {
  readonly reasons: string;
  readonly type: "liquidity_drain" | "stablecoin_depeg";
} {
  const reasons: string[] = [];
  if (signals.depeg !== null) {
    reasons.push(`stablecoin deviation ${(signals.depeg.deviationUsd * 100).toFixed(2)}%`);
  }
  if (signals.liquidityDrain !== null) {
    reasons.push(
      `TVL ${(signals.liquidityDrain.tvlPct * 100).toFixed(1)}%, volume ${(signals.liquidityDrain.volumePct * 100).toFixed(1)}%`,
    );
  }
  return {
    reasons: reasons.join("; "),
    type: signals.liquidityDrain !== null ? "liquidity_drain" : "stablecoin_depeg",
  };
}

// ─── Per-position REBALANCE decision builders ────────────────────────────────
// Pure functions for runRebalancePhase. Effectful recording (memory/audit
// alerts) stays in small nested sub-gens next to the loop.

/** Adopted EXTERNAL positions (pubkey set, no engine deposit) are manage-to-exit only. */
function isAdoptedExternal(
  positionPubKey: string | null | undefined,
  depositedUsd: number,
): boolean {
  return positionPubKey != null && depositedUsd <= 0;
}

function computeRebalanceDrift(input: {
  readonly activeBinId: number;
  readonly lowerBinId: number;
  readonly upperBinId: number;
}): { readonly positionCenter: number; readonly driftPct: number } {
  const positionCenter = (input.lowerBinId + input.upperBinId) / 2;
  const positionHalfWidth = (input.upperBinId - input.lowerBinId) / 2;
  return {
    positionCenter,
    driftPct: Math.abs(input.activeBinId - positionCenter) / (positionHalfWidth || 1),
  };
}

function shouldVolExitRebalance(input: {
  readonly highVol: boolean;
  readonly driftPct: number;
  readonly timeSinceRebal: number;
  ...
  return (
    input.highVol &&
    input.driftPct > 0.6 &&
    (input.timeSinceRebal >= input.minRebalanceIntervalMs || input.oorGraceExpired)
  );
}

function shouldAttemptRebalance(input: {
  readonly driftPct: number;
  readonly timeSinceRebal: number;
  readonly minRebalanceIntervalMs: number;
  readonly oorGraceExpired: boolean;
  ...
  return (
    (input.driftPct > 0.6 || input.oorGraceExpired) &&
    (input.timeSinceRebal >= input.minRebalanceIntervalMs || input.oorGraceExpired)
  );
}

/** Gas/recovery HOLD verdict for a rebalance candidate (null = proceed). */
type RebalanceHoldDecision =
  | {
      readonly kind: "gas";
  ...
  | { readonly kind: "recovery"; readonly recoveryProb: number; readonly holdThreshold: number };

function decideRebalanceGasHold(input: {
  readonly poolTvlUsd: number;
  readonly positionValueUsd: number;
  readonly poolFees24hUsd: number;
  readonly rebalanceGasCostNative: number;
  readonly nativePriceUsd: number;
  readonly minDaysOfFeesPaidAhead: number;
  readonly recoveryProb: number;
  readonly recoveryHoldThreshold: number;
}): RebalanceHoldDecision | null {
  const positionSharePct =
    input.poolTvlUsd > 0 && input.positionValueUsd > 0
      ? Math.min(input.positionValueUsd / input.poolTvlUsd, 1)
      : 0;
  const gasGate = evaluateGasGate({
    rebalanceGasCostNative: input.rebalanceGasCostNative,
    nativePriceUsd: input.nativePriceUsd,
    positionDailyFeesUsd: input.poolFees24hUsd * positionSharePct,
    minDaysOfFeesPaidAhead: input.minDaysOfFeesPaidAhead,
  });
  if (!gasGate.approved) {
    return {
      kind: "gas",
      reason: gasGate.reason,
      gasCostUsd: gasGate.gasCostUsd,
      feesThresholdUsd: gasGate.feesThresholdUsd,
    };
  }
  if (shouldHoldForRecovery(input.recoveryProb, input.recoveryHoldThreshold)) {
    return {
      kind: "recovery",
      recoveryProb: input.recoveryProb,
      holdThreshold: input.recoveryHoldThreshold,
    };
  }
  return null;
}

function shouldTakeRebalance(input: {
  readonly adoptedExternal: boolean;
  readonly netBenefitUsd: number;
  readonly minNetBenefitUsd: number;
  readonly recoveryProb: number;
  readonly forceThreshold: number;
}): { readonly take: boolean; readonly force: boolean } {
  const force = input.recoveryProb <= input.forceThreshold;
  return {
    take:
      !input.adoptedExternal &&
      (input.netBenefitUsd > input.minNetBenefitUsd || force),
    force,
  };
}

function buildHoldDecision(input: {
  readonly feeIlRatio: number;
  readonly minFeeIlRatio: number;
  readonly hasRecentWarning: boolean;
  readonly poolAddress: string;
  readonly positionId: string;
}): AgentDecision | null {
  if (input.feeIlRatio > input.minFeeIlRatio && !input.hasRecentWarning) {
    return {
      action: "HOLD",
      poolAddress: input.poolAddress,
      positionId: input.positionId,
      confidence: Math.min(0.6 + input.feeIlRatio * 0.05, 0.9),
      reasoning: `Fee/IL ${input.feeIlRatio.toFixed(2)} above threshold. Holding.`,
    };
  }
  return null;
}

function buildRotationChallengers(
  harvestBookStats: ReadonlyMap<
    string,
    { readonly tvlUsd: number; readonly apr: number; readonly fees24hUsd: number }
  >,
  poolAddress: string,
  maxEntrySizeUsd: number,
  entrySizeTvlFraction: number | undefined,
): Challenger[] {
  ...
  for (const [cAddr, cStats] of harvestBookStats) {
    ...
    const entryUsd = Math.min(maxEntrySizeUsd, (entrySizeTvlFraction ?? 0.005) * cStats.tvlUsd);
...
function selectBestRotationChallenger(
  challengers: ReadonlyArray<Challenger>,
  incumbentNet: number,
): Challenger | null {
  ...
  for (const c of challengers) {
    ...
    const bestApr = best?.apr ?? 0;
    if (margin > bestMargin || (margin === bestMargin && c.apr > bestApr)) {
      best = c;
      bestMargin = margin;
    }
  }
...
function buildRotationConfig(input: {
  readonly minSuperiorityPct: number | undefined;
  readonly minAprLeadPct: number | undefined;
  readonly minFeeDensityLeadPct: number | undefined;
  readonly requiredConfirmations: number | undefined;
  readonly switchingCostUsd: number | undefined;
}): RotationConfig {
  return {
    minSuperiorityPct: input.minSuperiorityPct ?? 25,
    minAprLeadPct: input.minAprLeadPct ?? 20,
    minFeeDensityLeadPct: input.minFeeDensityLeadPct ?? 15,
    requiredConfirmations: input.requiredConfirmations ?? 2,
    minChallengerNetFeesUsdPerDay: 0.05,
    switchingCostUsd: input.switchingCostUsd ?? 0.5,
  };
}

function isRotationSuperior(input: {
  readonly bestNet: number;
  readonly incumbentNet: number;
  readonly bestApr: number;
  readonly seatApr: number;
  readonly bestFeeDensity: number;
  readonly seatFeeDensity: number;
  readonly minSuperiorityPct: number | undefined;
  readonly minAprLeadPct: number | undefined;
  readonly minFeeDensityLeadPct: number | undefined;
  ...
  return (
    input.bestNet >= input.incumbentNet * (1 + (input.minSuperiorityPct ?? 25) / 100) &&
    input.bestApr >= input.seatApr + (input.minAprLeadPct ?? 20) &&
    input.bestFeeDensity >=
      input.seatFeeDensity * (1 + (input.minFeeDensityLeadPct ?? 15) / 100) &&
    input.bestNet >= input.minChallengerNetFeesUsdPerDay
  );
}

function isTrustedMint(mint: string, stablecoinMints: ReadonlySet<string> | undefined): boolean {
  return stablecoinMints?.has(mint) === true;
}

function isFreezeEnabled(auth: { readonly freezeAuthority: string | null } | null): boolean {
  return auth?.freezeAuthority != null;
}

function shouldAccruePaperFees(
  paperTrading: boolean | undefined,
  positionPubKey: string | null | undefined,
  statsSource: string | undefined,
): boolean {
  if (!paperTrading) return false;
  if (positionPubKey != null) return false;
  return statsSource === "datapi" || statsSource === "krystal";
}
/** Challenge-mode volatility signals: range override, trailing yield, rotation. */
type ChallengePoolSignals = {
  readonly rangeHalfWidthOverride: number | null;
  readonly avgYieldPerDayPct: number | null;
  readonly rotation: { readonly action: string; readonly reason: string } | null;
};

function resolveChallengeSignals(input: {
  readonly challengeMode: boolean | undefined;
  readonly pool: PoolState;
  readonly previousSnapshots: ReadonlyArray<PoolSnapshot>;
  readonly rangeVolK: number | undefined;
  readonly drawdownExitPct: number | undefined;
}): ChallengePoolSignals {
  if (input.challengeMode !== true || input.pool.statsSource !== "krystal") {
    return { rangeHalfWidthOverride: null, avgYieldPerDayPct: null, rotation: null };
  }
  const priceVolatility = input.pool.priceVolatility ?? 0;
  const rangeHalfWidthOverride =
    priceVolatility > 0
      ? challengeRangeFromVolatility(
      ...
      : null;
  const avgYieldPerDayPct = avgFeeYieldPct(input.previousSnapshots);
  const rotation = challengeRotationSignal(input.pool, avgYieldPerDayPct, input.drawdownExitPct ?? 5);
  return { rangeHalfWidthOverride, avgYieldPerDayPct, rotation };
}
...
export const program = Effect.gen(function* () {
  ...
  const prunePaperExitedPositions = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const loadPersistedCandidates = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const runStartupDiscovery = (): Effect.Effect<void, Error, never> =>
    Effect.gen(function* () {
  ...
  const warnIfSupervisedWithoutToken = (): void => {
    if (
      config.agentiveMode &&
  ...
  yield* prunePaperExitedPositions();
  // ─── Pool discovery ────────────────────────────────────────────────────────
  ...
  const assessMarketStressLoggingOnly = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      const baselineRaw = yield* db
        ...
        .pipe(Effect.catch(() => Effect.succeed(null)));
      const baseline: number[] = parseJsonOr(baselineRaw, []);
  ...
  const computeLvrPoolEdge = (
    ...
    if (!(latest.tvlUsd > 0) || !(latest.fees24hUsd > 0)) return null;
    ...
    return { pool, edge, feeYield: annualizedFeeYield, vol: variance };
  };

  const formatLvrEdgeSummary = (results: ReadonlyArray<LvrEdgeResult>) => ({
  ...
  const appendLvrHistory = (
    history: Record<string, LvrHistoryEntry[]>,
    results: ReadonlyArray<LvrEdgeResult>,
    now: number,
  ): void => {
  ...
  const assessLvrEdgeLoggingOnly = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      const results: LvrEdgeResult[] = [];
      for (const pool of pools) {
        const snaps = yield* db
          .getSnapshots(pool, now - (LVR_WINDOW_HOURS + 2) * 3_600_000, now)
          .pipe(Effect.catch(() => Effect.succeed([] as ReadonlyArray<PoolSnapshot>)));
        const poolEdge = computeLvrPoolEdge(pool, snaps, now);
        if (poolEdge === null) continue;
        results.push(poolEdge);
      }
      if (results.length === 0) return;
      results.sort((a, b) => b.edge - a.edge);
      logger.info("LVR edge assessed", formatLvrEdgeSummary(results));
      // Persist per-pool history for later validation against closed trades.
      const historyRaw = yield* db
        .getMetadata(LVR_META_KEY)
        .pipe(Effect.catch(() => Effect.succeed(null)));
      const history: Record<string, LvrHistoryEntry[]> = parseJsonOr(historyRaw, {});
      appendLvrHistory(history, results, now);
      yield* db
  ...
  const loadScreenedPools = (
    scanOrdinal: number,
  ): Effect.Effect<ReadonlyArray<ScreenedPool>, never, never> =>
    Effect.gen(function* () {
      if (config.marketScanEnabled === true && marketRankedPools.length > 0) {
        return marketRankedPools.slice(0, config.candidateScanLimit).map((rank) => ({
      ...
      return yield* screener.screenPools(scanOrdinal).pipe(
        Effect.catch((error) => {
  ...
  const resolveRouteAvailableMints = (
    mints: ReadonlyArray<string>,
    priceEvidence: ReadonlyArray<TokenPriceEvidence>,
  ): Effect.Effect<Set<string>, never, never> =>
    Effect.gen(function* () {
      const routeAvailableMints = new Set<string>();
      if (adapter.quoteSwap === undefined) return routeAvailableMints;
  ...
  const refreshAutonomousCandidates = (scanOrdinal: number): Effect.Effect<void, never> =>
    Effect.gen(function* () {
      ...
      const screened = yield* loadScreenedPools(scanOrdinal);
      const now = Date.now();
      ...
      const priceEvidence =
        adapter.getTokenPriceEvidence === undefined
          ...
          : yield* adapter
              ...
              .pipe(Effect.catch(() => Effect.succeed([])));
      const routeAvailableMints = yield* resolveRouteAvailableMints(mints, priceEvidence);
  ...
  const resolveIdleRedeployContext = (
    candidateCount: number,
  ): Effect.Effect<IdleRedeployContext | null, never, never> =>
    Effect.gen(function* () {
      ...
      return { idleCapitalUsd, portfolioValueUsd, recentPnlUsd, openPositions };
    });

  type RedeployGate =
    | { readonly outcome: "skip" }
    | { readonly outcome: "abort" }
    | { readonly outcome: "proceed"; readonly decision: AgentDecision };

  const recordRedeploySkipFor =
    (candidate: IdleRedeployCandidate, cycleId: string) =>
    (reasoning: string, riskReason: string) =>
      audit
        .recordDecision({
          timestamp: Date.now(),
          cycleId,
  ...
  const gateRedeployCandidate = (
    candidate: IdleRedeployCandidate,
    ctx: IdleRedeployContext,
    cycleId: string,
    executedExitPools: ReadonlySet<string>,
  ): Effect.Effect<RedeployGate, never, never> =>
    Effect.gen(function* () {
      const recordRedeploySkip = recordRedeploySkipFor(candidate, cycleId);
      ...
      const allocation: PerPoolAllocationResult = evaluatePerPoolAllocation({
      ...
      return { outcome: "proceed", decision };
    });
  ...
  const emptyRedeployOverlay: RedeployOverlayState = {
  ...
  type RedeployOverlay =
    | { readonly skipped: true }
    | {
        readonly skipped: false;
        readonly decision: AgentDecision;
        readonly overlay: RedeployOverlayState;
      };

  interface RedeployProposalSource {
    readonly agentProposal: AgentProposal | null;
    readonly proposalSource: "queue" | "sync" | undefined;
  }

  const applyVetoOverlay = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    overlayPoolAddress: string,
    overlayWarnings: ReadonlyArray<MemoryEntry>,
    overlayRecentDecisions: ReadonlyArray<DecisionRecord>,
    overlayHasOpenPosition: boolean,
  ): Effect.Effect<AgentDecision, never, never> =>
    Effect.gen(function* () {
      // SAFETY: this assertion is applied to a value whose producer and invariant are controlled by this code path.
  ...
  const resolveRedeployProposal = (
    ...
    overlayRecentDecisions: ReadonlyArray<DecisionRecord>,
    overlayHasOpenPosition: boolean,
  ): Effect.Effect<RedeployProposalSource, never, never> =>
    Effect.gen(function* () {
      ...
      return { agentProposal, proposalSource };
    });

  const applyResolvedProposal = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycleId: string,
    resolved: RedeployProposalSource,
    proposalMode: AgentProposalMode,
    now: number,
    ctx: IdleRedeployContext,
  ): Effect.Effect<RedeployOverlay, never, never> =>
    Effect.gen(function* () {
      const { agentProposal, proposalSource } = resolved;
      const overlayPoolAddress = candidate.poolAddress;
      if (!agentProposal) return { skipped: false, decision, overlay: emptyRedeployOverlay };
      const proposalToEvaluate: AgentProposal = {
        ...agentProposal,
        ...(agentProposal.originalAction === undefined ? { originalAction: decision.action } : {}),
      ...
      if (validation.valid && validation.adjustedDecision) {
        if (proposalMode === "suggest") {
          ...
          return { skipped: false, decision, overlay: emptyRedeployOverlay };
        }
        if (validation.adjustedDecision.action === "ENTER") {
          ...
          const overlayPreApplyDecision = decision;
          const nextDecision = validation.adjustedDecision;
          const overlayAppliedAgentProposal = decisionChangesExecutableBehavior(
            overlayPreApplyDecision,
            nextDecision,
            config.confidenceThreshold,
          );
          const overlayAppliedProposalId =
            proposalSource === "queue" && agentProposal.proposalId
          ...
          return {
            ...
            overlay: {
              appliedProposalId: overlayAppliedProposalId,
        ...
        yield* recordRedeploySkipFor(candidate, cycleId)(
  ...
  const checkRedeployOverlayHold = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycleId: string,
    ...
    Effect.gen(function* () {
      const recordSkip = recordRedeploySkipFor(candidate, cycleId);
      const overlayPoolAddress = candidate.poolAddress;
  ...
  const applyRedeployOverlay = (
    ...
    Effect.gen(function* () {
      ...
      // per-decision overlay the in-slot tail uses, BEFORE risk evaluation —
      ...
      let overlay = emptyRedeployOverlay;
      if (proposalMode === "veto") {
        nextDecision = yield* applyVetoOverlay(
          decision,
          candidate,
          overlayPoolAddress,
          overlayWarnings,
          overlayRecentDecisions,
          overlayHasOpenPosition,
        );
      } else {
        const resolved = yield* resolveRedeployProposal(
          decision,
          candidate,
          overlayPoolAddress,
          proposalMode,
        ...
        const applied = yield* applyResolvedProposal(
          decision,
          candidate,
          cycleId,
          resolved,
        ...
        if (applied.skipped) return { skipped: true };
        nextDecision = applied.decision;
        overlay = applied.overlay;
      }
      const held = yield* checkRedeployOverlayHold(
        nextDecision,
        candidate,
        cycleId,
      ...
      return { skipped: false, decision: nextDecision, overlay };
    });

  const isFundingCapacityFailure = (executionError: string | undefined): boolean =>
    solFundedEntryMode &&
  ...
  const evaluateRedeployRisk = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycleId: string,
    ctx: IdleRedeployContext,
    overlay: RedeployOverlayState,
  ): Effect.Effect<
    { approved: true; decision: AgentDecision; riskResult: RiskResult } | { approved: false },
    never,
    never
  > =>
    Effect.gen(function* () {
      ...
      let riskedDecision = decision;
      if (redeployCopySignalResult.boost > 0 && riskedDecision.action !== "EXIT") {
        riskedDecision = applyCopySignalBoost(
          riskedDecision,
      ...
      const pauseBlockReason = safetyPauseBlockReason(
        ...
        activeSafetyPause,
        riskedDecision.action,
      );
      const riskResult: RiskResult =
        pauseBlockReason === null
          ? risk.evaluate(riskedDecision, riskCtx)
          : {
              approved: false,
      ...
      if (riskResult.adjustedSizeUsd) {
        riskedDecision = {
          ...
          reasoning: `${riskedDecision.reasoning} (size capped to $${riskResult.adjustedSizeUsd.toFixed(0)})`,
        };
      }
      if (!riskResult.approved) {
        ...
        yield* audit
          .recordDecision({
            ...
            poolAddress: candidate.poolAddress,
            action: "ENTER",
            confidence: riskedDecision.confidence,
            reasoning: riskedDecision.reasoning,
            metrics: candidate.metrics,
            riskResult,
            executed: false,
            paperTrading: config.paperTrading,
        ...
        const penalizeAppliedProposal = shouldPenalizeAppliedProposalDenial({
          appliedAgentProposal: overlay.appliedAgentProposal,
          preApplyDecision: overlay.preApplyDecision,
          appliedDecision: riskedDecision,
          isPreApplyRiskApproved: () =>
            overlay.preApplyDecision !== undefined &&
            risk.evaluate(overlay.preApplyDecision, riskCtx).approved,
        });
        yield* recordAppliedProposalRiskDenial(agentState, {
          penalizeAdvisor: penalizeAppliedProposal,
          appliedQueuedProposalId: overlay.appliedProposalId,
          proposalBackoff,
      ...
      recordAppliedProposalRiskApproval({
        proposalValidated: overlay.proposalValidated,
        proposalBackoff,
        recordCircuitSuccess: () => getPoolCircuitBreaker(candidate.poolAddress).recordSuccess(),
        poolAddress: candidate.poolAddress,
      });
      return { approved: true as const, decision: riskedDecision, riskResult };
    });

  const checkWidenedSize = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycleId: string,
  ): Effect.Effect<boolean, never, never> =>
  ...
  ): Effect.Effect<RedeployExecution, never, never> =>
    Effect.gen(function* () {
      ...
      return { outcome: "done", executed: liveResult.executed, executionError: liveResult.error };
    });

  const dispatchRedeployExecution = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycleId: string,
    signalTimestamp: number,
    ...
    Effect.gen(function* () {
      ...
      if (config.autonomousTokenMode === "shadow") {
      ...
      return yield* executeLiveRedeploy(
        decision,
        candidate,
        cycleId,
        entryStrategyMode,
  ...
  const finalizeRedeployOutcome = (
    decision: AgentDecision,
    candidate: IdleRedeployCandidate,
    cycle: AgentCycle,
    idleCapitalUsd: number,
    overlayAppliedProposalId: string | undefined,
    ...
    Effect.gen(function* () {
      ...
      } else if (!isFundingCapacityFailure(executionError)) {
  ...
  const runIdleRedeployPass = (
    ...
    Effect.gen(function* () {
      ...
      const ctx = yield* resolveIdleRedeployContext(candidates.length);
      if (!ctx) return;
      const { idleCapitalUsd } = ctx;
      ...
      for (const candidate of sortedCandidates) {
        ...
        // positions). The fresh position-count gates alone miss it: the exit freed
        // the slot, so the count re-check passes. Exclude by executed-exit pool.
        const gated = yield* gateRedeployCandidate(candidate, ctx, cycleId, executedExitPools);
        if (gated.outcome === "abort") return;
        if (gated.outcome === "skip") continue;
        ...
        // shared functions in the SAME order as the in-slot tail. AGENTIC_MODE=false
        // (default) skips the whole block → zero behavior change.
        const overlaid = yield* applyRedeployOverlay(decision, candidate, cycleId, ctx);
        if (overlaid.skipped) continue;
        decision = overlaid.decision;
        const { appliedProposalId: overlayAppliedProposalId } = overlaid.overlay;
        const risked = yield* evaluateRedeployRisk(
          decision,
          candidate,
          cycleId,
          ctx,
        ...
        if (!risked.approved) continue;
        decision = risked.decision;

        if (!(yield* checkWidenedSize(decision, candidate, cycleId))) continue;
        ...
        const exec = yield* dispatchRedeployExecution(
          decision,
          candidate,
          cycleId,
          signalTimestamp,
        ...
        if (exec.outcome === "stop") return;
        if (exec.outcome === "next") continue;
  ...
  const refreshMarketUniverse = (now: number): Effect.Effect<void, never> =>
    Effect.gen(function* () {
      ...
      const { ranked } = gateAndRankMarketPools(discovered, buildMarketGateConfig(config));
      marketRankedPools = ranked;
      const activeCount = marketScanActiveCount(config);
      ...
      logger.info("Market scan: universe gate refreshed", {
        ...
        top: formatTopMarketRanks(ranked),
      });
    });
  ...
  const processSettlementPhase = (): Effect.Effect<number, never, never> =>
    Effect.gen(function* () {
      let oldestSettlementAgeMs = 0;
      if (!autonomousExecution) return oldestSettlementAgeMs;
  ...
  const resolveCycleExecutionFailuresPause = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const refreshCycleWallet = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const hydrateChallengePeak = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const hydrateChallengeCooldowns = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      // hydrate at boot.
      if (config.challengeMode !== true || challengeLossCooldownUntil.size > 0) return;
      const persistedCooldowns = yield* db
        .getMetadata("challenge_loss_cooldowns")
        .pipe(Effect.catch(() => Effect.succeed(null)));
      if (persistedCooldowns === null) return;
      try {
        for (const [pool, until] of Object.entries(
          parseJsonOr<Record<string, unknown>>(persistedCooldowns, {}),
        )) {
  ...
  const hydrateChallengeFirstSeen = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      if (config.challengeMode !== true || poolFirstSeenAt.size > 0) return;
      const persistedFirstSeen = yield* db
        .getMetadata("challenge_pool_first_seen")
        .pipe(Effect.catch(() => Effect.succeed(null)));
      if (persistedFirstSeen === null) return;
      try {
        for (const [pool, seenAt] of Object.entries(
          parseJsonOr<Record<string, unknown>>(persistedFirstSeen, {}),
        )) {
  ...
  const hydrateChallengeRotations = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      if (config.challengeMode !== true || rotationConfirmations.size > 0) return;
  ...
  const refreshCycleSolBudget = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const logWalletComposition = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const runPoolScanLoop = (
    cycle: AgentCycle,
    idleRedeployCandidates: IdleRedeployCandidate[],
    executedExitPools: Set<string>,
    getPrefetched: (poolAddress: string) => { state: PoolState; bins: BinArray | null } | undefined,
  ): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const pruneAndReportCycle = (cycle: AgentCycle): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
  ...
  const runScanCycle = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      rebuildPoolsToScan();
      const oldestSettlementAgeMs = yield* processSettlementPhase();
      ...
      yield* refreshCycleWallet();

      yield* hydrateChallengePeak();

      yield* hydrateChallengeCooldowns();

      yield* hydrateChallengeFirstSeen();

      yield* hydrateChallengeRotations();
      // Issue #170: batch wallet-reserve gate — refresh the per-cycle native
      // SOL budget for SOL-funded entries. One read, reused by every ENTER
      // gate this cycle; a failed read leaves the budget UNKNOWN and the gate
      // skips entries fail-closed (never commit SOL the engine cannot confirm).
      ...
      const prefetched =
        adapter.getPoolStatesWithBins !== undefined
          ? yield* adapter
              ...
              .pipe(Effect.catch(() => Effect.succeed(null)))
          : null;
      const getPrefetched = (poolAddress: string) => prefetched?.get(poolAddress.toLowerCase());
      yield* runPoolScanLoop(cycle, idleRedeployCandidates, executedExitPools, getPrefetched);

      yield* runRedeployAndDecay(
        cycle,
        idleRedeployCandidates,
        executedExitPools,
        oldestSettlementAgeMs,
      ...
      yield* pruneAndReportCycle(cycle);
    });
  ...
  const evaluatePool = (
    ...
    Effect.gen(function* () {
      ...
      const ensureFirstSeenStage = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
      ...
      yield* ensureFirstSeenStage();

      const fetchPoolStage = (): Effect.Effect<
        { readonly rawPool: PoolState; readonly binArray: BinArray },
        Error
      > =>
        Effect.gen(function* () {
          const rawPool =
          ...
          return { rawPool, binArray };
        });
      const fetched = yield* fetchPoolStage();
      const rawPool = fetched.rawPool;
      const binArray = fetched.binArray;
      ...
      const resolveStatsStage = (rawPool: PoolState): Effect.Effect<PoolState, Error> =>
        Effect.gen(function* () {
          const krystalStats =
          ...
          const geckoStats =
            krystalStats === null && config.geckoTerminalEnabled !== false
              ...
              : null;
          if (krystalStats !== null) return enrichPoolFromKrystal(rawPool, krystalStats);
          if (geckoStats !== null) return enrichPoolFromGecko(rawPool, geckoStats);
          return rawPool;
        });
      const pool = yield* resolveStatsStage(rawPool);
      ...
      const backdateLaunchRugStage = (pool: PoolState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (
      ...
      yield* backdateLaunchRugStage(pool);
      ...
      const snapshotStage = (
        ...
        {
          readonly previousSnapshots: ReadonlyArray<PoolSnapshot>;
          readonly previousSnapshot: PoolSnapshot | undefined;
          readonly w15Signals: DepegLiquiditySignals;
        },
        Error
      > =>
        Effect.gen(function* () {
          ...
          return { previousSnapshots, previousSnapshot, w15Signals };
        });
      const snapshots = yield* snapshotStage(pool);
      const previousSnapshots = snapshots.previousSnapshots;
      const previousSnapshot = snapshots.previousSnapshot;
      const w15Signals = snapshots.w15Signals;
      ...
      const checkBlacklistStage = (
        pool: PoolState,
        authX: { mintAuthority: string | null; freezeAuthority: string | null } | null,
        authY: { mintAuthority: string | null; freezeAuthority: string | null } | null,
      ): Effect.Effect<ReadonlyArray<AgentDecision> | null, Error> =>
        Effect.gen(function* () {
      ...
      const blacklistHalt = yield* checkBlacklistStage(pool, authX, authY);
      if (blacklistHalt !== null) return blacklistHalt;
      ...
      const checkTokenRiskStage = (
        pool: PoolState,
      ): Effect.Effect<ReadonlyArray<AgentDecision> | null, Error> =>
        Effect.gen(function* () {
          for (const mint of [pool.tokenX, pool.tokenY]) {
            if (mint === NATIVE_MINT || isTrustedMint(mint, config.stablecoinMints)) continue;
      ...
      const tokenRiskHalt = yield* checkTokenRiskStage(pool);
      if (tokenRiskHalt !== null) return tokenRiskHalt;
      ...
      const checkFreezeStage = (
        pool: PoolState,
        authX: { mintAuthority: string | null; freezeAuthority: string | null } | null,
        authY: { mintAuthority: string | null; freezeAuthority: string | null } | null,
      ): Effect.Effect<ReadonlyArray<AgentDecision> | null, Error> =>
        Effect.gen(function* () {
          const freezeEnabledX = isFreezeEnabled(authX);
          const freezeEnabledY = isFreezeEnabled(authY);
          const trustedX = isTrustedMint(pool.tokenX, config.stablecoinMints);
          const trustedY = isTrustedMint(pool.tokenY, config.stablecoinMints);
          ...
          return yield* adjudicateFreezeLegs(pool, untrustedFreezeX, untrustedFreezeY);
        });
      const freezeHalt = yield* checkFreezeStage(pool, authX, authY);
      if (freezeHalt !== null) return freezeHalt;
      ...
      const metricsStage = (
        ...
        | { readonly halt: ReadonlyArray<AgentDecision> }
        | {
            readonly halt: null;
            readonly metrics: PoolMetrics;
            readonly feeIlRatio: number;
            ...
            readonly poolPositions: PositionRecord[];
          },
        Error
      > =>
        Effect.gen(function* () {
          ...
          if (
            ...
            return { halt: [] as ReadonlyArray<AgentDecision> };
          }
          ...
          const poolPositions = positionsForPool(trackedPositions, poolAddress).sort(
          ...
          return {
            ...
            volumeAuth: metrics.volumeAuthenticity,
      ...
      const measured = yield* metricsStage(pool, binArray, previousSnapshot);
      if (measured.halt !== null) return measured.halt;
      const metrics = measured.metrics;

      // Decision rules
      const feeIlRatio = measured.feeIlRatio;
      const volumeAuth = measured.volumeAuth;
      const tvlVelocity = measured.tvlVelocity;
      const binUtilization = measured.binUtilization;
      const hasRecentWarning = measured.hasRecentWarning;
      const warnings = measured.warnings;
      let poolPositions = measured.poolPositions;
      ...
      const syncOorTrackingStage = (pool: PoolState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
      ...
      yield* syncOorTrackingStage(pool);
      ...
      const reconcileExternalClosesStage = (
        poolPositions: PositionRecord[],
      ): Effect.Effect<PositionRecord[], Error> =>
        Effect.gen(function* () {
      ...
      poolPositions = yield* reconcileExternalClosesStage(poolPositions);

      const volatilityStage = (
        pool: PoolState,
        previousSnapshots: ReadonlyArray<PoolSnapshot>,
      ): Effect.Effect<
        {
          readonly recentBins: number[];
          readonly volatilityBins: number[];
          readonly volatilityStddev: number;
          readonly rangeHalfWidth: number;
          readonly challengeRotation: ChallengePoolSignals["rotation"];
          readonly challengeAvgYieldPerDayPct: ChallengePoolSignals["avgYieldPerDayPct"];
          readonly netDriftBins: number;
        },
        Error
      > =>
        Effect.gen(function* () {
          ...
          const challengeSignals = resolveChallengeSignals({
            challengeMode: config.challengeMode,
            pool,
            previousSnapshots,
            rangeVolK: config.challengeRangeVolK,
            drawdownExitPct: config.challengeDrawdownExitPct,
          });
          // Use the volatility-sized width for ENTER/REBALANCE when present.
          if (challengeSignals.rangeHalfWidthOverride !== null) {
            rangeHalfWidth = challengeSignals.rangeHalfWidthOverride;
          }
          ...
          return {
            ...
            volatilityStddev,
            rangeHalfWidth,
            challengeRotation: challengeSignals.rotation,
            challengeAvgYieldPerDayPct: challengeSignals.avgYieldPerDayPct,
            netDriftBins,
          };
        });
      const vol = yield* volatilityStage(pool, previousSnapshots);
      const recentBins = vol.recentBins;
      const volatilityBins = vol.volatilityBins;
      const volatilityStddev = vol.volatilityStddev;
      const rangeHalfWidth = vol.rangeHalfWidth;
      const challengeRotation = vol.challengeRotation;
      const challengeAvgYieldPerDayPct = vol.challengeAvgYieldPerDayPct;
      const netDriftBins = vol.netDriftBins;
      ...
      const syncPositionValuesStage = (pool: PoolState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          const resolvePositionMark = (pos: PositionRecord): Effect.Effect<number | null, Error> =>
            Effect.gen(function* () {
              if (pos.positionPubKey == null) return null;
              if (
                positionValuesThisCycle !== null &&
                positionValuesThisCycle.has(pos.positionPubKey)
              ) {
                return positionValuesThisCycle.get(pos.positionPubKey) ?? null;
              }
              if (adapter.getPositionValueUsd != null) {
          ...
          const accruePaperFees = (
            pos: PositionRecord,
            pool: PoolState,
          ): Effect.Effect<void, Error> =>
            Effect.gen(function* () {
              if (!shouldAccruePaperFees(config.paperTrading, pos.positionPubKey, pool.statsSource))
                return;
              const now = Date.now();
          ...
          for (const pos of poolPositions) {
            const realMark = yield* resolvePositionMark(pos);
            ...
            yield* accruePaperFees(pos, pool);
            yield* persist(`savePosition ${pos.positionId}`, db.savePosition(pos));
          }
        });
      yield* syncPositionValuesStage(pool);
      ...
      const runExitPhase = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (w15Signals.depeg || w15Signals.liquidityDrain) {
            const w15Alert = describeW15Alert(w15Signals);
            yield* alertSvc.sendAlert({
              type: w15Alert.type,
              severity: "critical",
              message: `Fast EXIT signal on ${pool.tokenXSymbol}/${pool.tokenYSymbol}: ${w15Alert.reasons}`,
              poolAddress,
              data: { ...w15Signals },
            });
          }

          const dispatchExitBranchSideEffect = (
            outcome: PositionExitBranchOutcome,
            pos: PositionRecord,
          ): Effect.Effect<void, Error> =>
            Effect.gen(function* () {
              if (outcome.ilAlert !== null) {
                yield* alertSvc.sendAlert({
                  type: "il_dominance",
                  severity: "critical",
                  message: outcome.ilAlert.message,
                  poolAddress,
                  positionId: pos.positionId,
                  data: {
                    ilUsd: outcome.ilAlert.ilUsd,
                    hodlValueUsd: outcome.ilAlert.hodlValueUsd,
                    feesClaimedUsd: outcome.ilAlert.feesClaimedUsd,
                  },
                });
              }
              if (outcome.memoryWarning !== null && !poolExitFired) {
                yield* memory
                  .upsert({
                    category: "warning",
                    content: outcome.memoryWarning,
                    poolAddress,
                  })
                  .pipe(Effect.catch(() => Effect.void));
              }
              if (outcome.agentAlert !== null) {
                yield* sendAgentAlert(
                  outcome.agentAlert.severity,
                  outcome.agentAlert.category,
                  outcome.agentAlert.message,
          ...
          for (const pos of poolPositions) {
            ...
            let decision: AgentDecision | null = buildChallengeRotationExit(
              challengeRotation,
              poolAddress,
              pos.positionId,
            );

            // Strategy table: the first non-null branch wins. Priority order
            // matches the legacy else-if chain (fallen-angel lifecycle > W15
            // fast EXIT > IL dominance > pool degradation).
            if (decision === null) {
              const exitBranches: Array<() => PositionExitBranchOutcome | null> = [
                () => {
                  const lifecycle = evaluateExitFallenAngel({
                    fallenAngelEnabled: config.fallenAngelEnabled === true,
                    positionMode: pos.positionMode,
                    tpLadderJson: pos.tpLadderJson,
                    invalidationStopPrice: pos.invalidationStopPrice,
                    currentPrice: pool.currentPrice,
                  });
                ...
                () => buildW15ExitOutcome(w15Signals, poolAddress, pos.positionId),
                () =>
                  buildIlDominanceOutcome({
                    ilProtectionEnabled: config.ilProtectionEnabled === true,
                    ...
                    cumulativeFeesClaimedUsd: pos.cumulativeFeesClaimedUsd,
                    currentPrice: pool.currentPrice,
                    dominanceFactor: config.ilDominanceExitFactor,
                    minUsd: config.ilDominanceMinUsd,
                    poolAddress,
                    positionId: pos.positionId,
                    tokenXSymbol: pool.tokenXSymbol,
                    tokenYSymbol: pool.tokenYSymbol,
                ...
                () =>
                  buildPoolDegradationOutcome({
                    dustExitUsd: config.dustExitUsd,
                    currentValueUsd: pos.currentValueUsd,
                    tvlVelocity,
                    tvlDropExitPct: config.tvlDropExitPct,
                    volumeAuthenticityKnown: metrics.volumeAuthenticityKnown,
                    volumeAuthenticity: volumeAuth,
                    volumeAuthThreshold: evolvedThresholds.volumeAuthThreshold,
                    feeIlRatioKnown: metrics.feeIlRatioKnown,
                    feeIlRatio,
                    poolAddress,
                    positionId: pos.positionId,
              ...
              for (const runBranch of exitBranches) {
                const outcome = runBranch();
                if (outcome !== null) {
                  decision = outcome.decision;
                  yield* dispatchExitBranchSideEffect(outcome, pos);
                  break;
                }
              }
            }
            ...
            const applyExitHardStop = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                if (!decision && pos.depositedUsd > 0) {
                  ...
                  const breaches = breached
                    ? (stopLossBreachCount.get(pos.positionId) ?? 0) + 1
                    : 0;
            ...
            yield* applyExitHardStop();

            // Trailing exit (profit protection)
            const applyExitTrailingStop = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                if (!decision) {
                  ...
                  const breaches = breached
                    ? (trailingStopBreachCount.get(pos.positionId) ?? 0) + 1
                    : 0;
                  ...
                  if (breached && breaches >= config.trailingStopConfirmCycles) {
                    ...
                    const pnlPct =
                      pos.depositedUsd > 0
                        ? (estimatedValue - pos.depositedUsd) / pos.depositedUsd
                        : 0;
                    if (pnlPct < -0.15) {
                      yield* sendAgentAlert(
                        "warning",
      ...
      const rolloverDayBaselineStage = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (dayRolledOver) {
      ...
      yield* rolloverDayBaselineStage();
      ...
      const autoResolvePausesStage = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (
            autonomousExecution &&
            activeSafetyPause !== null &&
      ...
      yield* autoResolvePausesStage();
      const armDrawdownPauseStage = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (
            autonomousExecution &&
            activeSafetyPause?.resolvedAt !== null &&
      ...
      const runRebalancePhase = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          ...
          const resolveRebalanceRange = (): BinRange => {
            const recommended = config.volatilityAdaptiveRanges
              ...
              : highVol
                ? recommendBinRangeForVolatility(
                    ...
                    config.entryRangeHalfWidthBins > 0
                      ? config.entryRangeHalfWidthBins
                      : undefined,
                  )
                : strategy.recommendBinRange(pool.activeBinId, pool.binStep, rangeHalfWidth);
          ...
          const simulateRebalanceBenefit = (
            pos: PositionRecord,
            range: { readonly lowerBinId: number; readonly upperBinId: number },
          ) => {
            if (config.paperTrading) {
              return estimatePaperRebalanceBenefit({
                fees24hUsd: pool.fees24hUsd,
                newLowerBinId: range.lowerBinId,
                newUpperBinId: range.upperBinId,
              });
            }
            if (pos.positionPubKey) {
              return estimateLiveRebalanceBenefit({
                fees24hUsd: pool.fees24hUsd,
                newLowerBinId: range.lowerBinId,
                newUpperBinId: range.upperBinId,
          ...
          const recordRebalanceHold = (hold: RebalanceHoldDecision): Effect.Effect<void, Error> =>
            Effect.gen(function* () {
              if (hold.kind === "gas") {
                console.info(
                  `[gas-gate] Holding ${poolAddress} — ${hold.reason} (gas=$${hold.gasCostUsd.toFixed(2)}, threshold=$${hold.feesThresholdUsd.toFixed(2)})`,
                );
                yield* memory
                  .upsert({
                    category: "warning",
                    content: `Gas-aware rebalance gate held ${poolAddress}: ${hold.reason}`,
                    poolAddress,
                  })
                  .pipe(Effect.catch(() => Effect.void));
                yield* audit
                  .recordDecision({
                    ...
                    action: "HOLD",
                    confidence: 0,
                    reasoning: `[gas-gate] ${hold.reason}`,
                    metrics,
                    riskResult: { approved: false, reason: `[gas-gate] ${hold.reason}` },
                    executed: false,
                    paperTrading: config.paperTrading,
                  })
                  .pipe(Effect.catch(() => Effect.void));
                ...
                console.info(
                  `[recovery-gate] Holding ${poolAddress} — recovery prob ${hold.recoveryProb.toFixed(2)} >= ${hold.holdThreshold}`,
                );
                yield* memory
                  .upsert({
                    category: "pattern",
                    content: `OOR recovery prediction held ${poolAddress}: probability ${hold.recoveryProb.toFixed(2)}`,
                    poolAddress,
                  })
                  .pipe(Effect.catch(() => Effect.void));
                yield* audit
                  .recordDecision({
                    ...
                    poolAddress,
                    action: "HOLD",
                    confidence: hold.recoveryProb,
                    reasoning: `[recovery-gate] probability ${hold.recoveryProb.toFixed(2)} >= ${hold.holdThreshold} — expecting mean-reversion`,
                    metrics,
                    riskResult: {
                      approved: false,
                      reason: `[recovery-gate] probability ${hold.recoveryProb.toFixed(2)} above hold threshold`,
                    },
                    executed: false,
                    paperTrading: config.paperTrading,
                  })
          ...
          const attemptRebalance = (
            pos: PositionRecord,
            driftPct: number,
            positionCenter: number,
          ): Effect.Effect<AgentDecision | null, Error> =>
            Effect.gen(function* () {
              const range = resolveRebalanceRange();
              const sim = simulateRebalanceBenefit(pos, range);
              if (sim === null) {
                yield* memory
                  .upsert({
                    category: "warning",
              ...
              const hold = decideRebalanceGasHold({
                poolTvlUsd: pool.tvlUsd,
                positionValueUsd: pos.currentValueUsd,
                poolFees24hUsd: pool.fees24hUsd,
                rebalanceGasCostNative: config.rebalanceGasCostNative,
                nativePriceUsd: config.nativePriceUsd,
                minDaysOfFeesPaidAhead: config.gasAwareMinDaysOfFeesPaidAhead,
                recoveryProb,
                recoveryHoldThreshold: config.oorRecoveryHoldThreshold,
              });
              if (hold !== null) {
                yield* recordRebalanceHold(hold);
                return null;
              }
              const take = shouldTakeRebalance({
                adoptedExternal: isAdoptedExternal(pos.positionPubKey, pos.depositedUsd),
                netBenefitUsd: sim.netBenefitUsd,
                minNetBenefitUsd: config.minRebalanceNetBenefitUsd,
                recoveryProb,
                forceThreshold: config.oorRecoveryForceRebalanceThreshold,
              });
              if (take.take) {
                return {
                  action: "REBALANCE",
          ...
          const evaluateRotationChallenge = (pos: PositionRecord): AgentDecision | null => {
            if (
              config.challengeMode !== true ||
            ...
            const challengers = buildRotationChallengers(
              harvestBookStats,
              poolAddress,
              config.maxEntrySizeUsd,
              config.entrySizeTvlFraction,
            );
            ...
            const bestChallenger = selectBestRotationChallenger(challengers, incumbentNet);
            if (bestChallenger === null) return null;
            const rotCfg = buildRotationConfig({
              minSuperiorityPct: config.rotationMinSuperiorityPct,
              minAprLeadPct: config.rotationMinAprLeadPct,
              minFeeDensityLeadPct: config.rotationMinFeeDensityLeadPct,
              requiredConfirmations: config.rotationRequiredConfirmations,
              switchingCostUsd: config.rotationSwitchingCostUsd,
            });
            ...
            const superior = isRotationSuperior({
              bestNet,
              incumbentNet,
              bestApr: bestChallenger.apr,
              seatApr: seat.apr,
              bestFeeDensity: bestChallenger.feeDensity,
              seatFeeDensity: seat.feeDensity,
              minSuperiorityPct: rotCfg.minSuperiorityPct,
              minAprLeadPct: rotCfg.minAprLeadPct,
              minFeeDensityLeadPct: rotCfg.minFeeDensityLeadPct,
              minChallengerNetFeesUsdPerDay: rotCfg.minChallengerNetFeesUsdPerDay,
            });
          ...
          for (const pos of poolPositions) {
            ...
            const { positionCenter, driftPct } = computeRebalanceDrift({
              activeBinId: pool.activeBinId,
              lowerBinId: pos.lowerBinId,
              upperBinId: pos.upperBinId,
            });
            ...
            if (
              ...
              yield* recordVolExit(pos, driftPct);
            } else if (
              shouldAttemptRebalance({
                driftPct,
                timeSinceRebal,
                minRebalanceIntervalMs: config.minRebalanceIntervalMs,
                oorGraceExpired,
              ...
              decision = yield* attemptRebalance(pos, driftPct, positionCenter);
            }
            ...
            if (decision === null) {
              decision = buildHoldDecision({
                feeIlRatio,
                minFeeIlRatio: evolvedThresholds.minFeeIlRatio,
                hasRecentWarning,
                poolAddress,
                positionId: pos.positionId,
              });
            ...
            if (decision === null) {
              decision = evaluateRotationChallenge(pos);
            }
      ...
      const runEnterSlot = (): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          if (!poolExitFired && poolPositions.length < config.maxPositionsPerPool) {
            if (unresolvedPoolAddresses.has(poolAddress)) {
              ...
              const runEnterWalletReadGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
              ...
              yield* runEnterWalletReadGate();

              const runEnterWalletRefreshGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
              ...
              yield* runEnterWalletRefreshGate();

              const runEnterBackoffGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                  const entryBackoff = entryFailureBackoff.get(poolAddress);
                  if (
                    !enterGateRejected &&
                    entryBackoff &&
                    entryBackoff.nextAttemptAt > Date.now()
                  ) {
              ...
              yield* runEnterBackoffGate();

              const runEnterCooldownGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                    ...
                    if (cooldown && Date.now() < cooldown.cooldownUntil) {
                      ...
                      yield* audit
                        .recordDecision({
                          ...
                          riskResult: {
                            approved: false,
                            reason: `[cooldown-gate] ${cooldown.reason}`,
                          },
                          executed: false,
                          paperTrading: config.paperTrading,
                        })
              ...
              const runFallenAngelEnter = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                    ...
                    if (!faAllocation.approved) {
                      ...
                      yield* audit
                        .recordDecision({
                          ...
                          riskResult: {
                            approved: false,
                            reason: `[fa-alloc-gate] ${faAllocation.reason}`,
                          },
                          executed: false,
                          paperTrading: config.paperTrading,
                        })
              ...
              yield* runFallenAngelEnter();

              const runEnterFeeIlGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
              ...
              yield* runEnterFeeIlGate();

              const runEnterFeeGasGate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
              ...
              const fetchEnterProofs = () =>
                Effect.gen(function* () {
                  ...
                  const applyEnterFastLane = (): Effect.Effect<void, Error> =>
                    Effect.gen(function* () {
                      ...
                      if (
                        ...
                        pool.volume24hUsd > 0 &&
                        pool.volume5mUsd >=
                          (pool.volume24hUsd / 288) * (config.challenge5mMultiplier ?? 3)
                      ) {
                        persistenceDecision = {
                          persistent: true,
                  ...
                  const exitRouteProof =
                    ...
                    harvestBookPools.has(poolAddress.toLowerCase())
                      ? adapter.verifyExitRoute
                        ...
                        : {
                            ok: false,
                            reason: "verifyExitRoute not implemented",
                            proceedsUsd: null,
                          }
                      : { ok: true, reason: "not required", proceedsUsd: null };
                  return { persistenceDecision, exitRouteProof };
                });
              const { persistenceDecision, exitRouteProof } = yield* fetchEnterProofs();
              ...
              const runEnterV4Gate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
              ...
              yield* runEnterV4Gate();
              const runEnterLiquidityFundingGates = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                  if (!enterGateRejected && pool.liquidity !== undefined && pool.liquidity <= 0n) {
                    yield* audit
                      .recordDecision({
                        ...
                        riskResult: {
                          approved: false,
                          reason: "[liquidity] zero on-chain liquidity",
                        },
                        executed: false,
                        paperTrading: config.paperTrading,
                      })
              ...
              yield* runEnterLiquidityFundingGates();
              const runEnterCandidate = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                  const isBaseEnterEligible = (): boolean =>
                    !enterGateRejected &&
                    (metrics.feeIlRatioKnown
                    ...
                    binUtilization > 0.4 &&
                    pool.tvlUsd > config.minPoolTvlUsd * 2;
                  const isChallengeYieldEligible = (): boolean =>
                    (config.challengeMode !== true ||
                    ...
                    (config.challengeMode !== true ||
                      pool.tvlUsd >= (config.minEntryTvlUsd ?? 1_000));
                  const isChallengeSafetyEligible = (): boolean =>
                  ...
                  if (
                    isBaseEnterEligible() &&
                    isChallengeYieldEligible() &&
                    isChallengeSafetyEligible()
                  ) {
                    const entryScore = weightedEntryScore(metrics, signalWeights);
                    if (entryScore <= config.weightedEntryScoreThreshold) {
                      const rejectLowScoreEnter = (): Effect.Effect<void, Error> =>
                        Effect.gen(function* () {
                          yield* audit
                            .recordDecision({
                              timestamp: Date.now(),
                      ...
                      if (!allocation.approved) {
                        const rejectAllocBlockedEnter = (): Effect.Effect<void, Error> =>
                          Effect.gen(function* () {
                            console.info(
                            ...
                            yield* audit
                              .recordDecision({
                                ...
                                riskResult: {
                                  approved: false,
                                  reason: `[alloc-gate] ${allocation.reason}`,
                                },
                                executed: false,
                                paperTrading: config.paperTrading,
                              })
                              .pipe(Effect.catch(() => Effect.void));
                            const captureAllocBlockedCandidate = (): Effect.Effect<void, Error> =>
                              Effect.gen(function* () {
                            ...
                            yield* captureAllocBlockedCandidate();
                            enterGateRejected = true;
                          });
                        yield* rejectAllocBlockedEnter();
                      } else {
                        const runEnterTokenRiskGate = (): Effect.Effect<void, Error> =>
                          Effect.gen(function* () {
                        ...
                        yield* runEnterTokenRiskGate();

                        const pushEnterDecision = (): Effect.Effect<void, Error> =>
                          Effect.gen(function* () {
                            if (!enterGateRejected) {
                        ...
                        yield* pushEnterDecision();
                      }
                    }
                  }
                });
              yield* runEnterCandidate();
            }
          }
        });
      yield* runEnterSlot();
      ...
      const maybeCaptureIdleRedeploy = (): void => {
        if (
          config.idleRedeployEnabled &&
          !poolExitFired &&
      ...
      maybeCaptureIdleRedeploy();

      const ensureDefaultHoldStage = (): Effect.Effect<
        ReadonlyArray<AgentDecision> | null,
        Error
      > =>
        Effect.gen(function* () {
      ...
      const defaultHoldHalt = yield* ensureDefaultHoldStage();
      if (defaultHoldHalt !== null) return defaultHoldHalt;

      const runDecisionTail = (): Effect.Effect<ReadonlyArray<AgentDecision>, Error> =>
        Effect.gen(function* () {
          ...
          for (const rawDecision of rawDecisions) {
            ...
            const findTailDecisionPosition = () =>
              decision.positionId !== undefined
                ? trackedPositions.get(decision.positionId)
                : undefined;
            const pos = findTailDecisionPosition();
            ...
            const resolveTailAgentOverlay = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                if (config.agentiveMode) {
                  const proposalMode = config.agentProposalMode;
                  const now = Date.now();
                  ...
                  if (proposalMode === "veto") {
                    const resolveTailVeto = (): Effect.Effect<void, Error> =>
                      Effect.gen(function* () {
                        ...
                        const enhanced = yield* agent
                          ...
                          .pipe(
                            ...
                            Effect.catch((err) => {
                              ...
                              vetoWarnEligible =
                                now - (vetoWarningThrottle.get(poolAddress) ?? 0) >
                                config.agentProposalStaleMs;
                              if (vetoWarnEligible) {
                    ...
                    const fetchTailSyncProposal = (): Effect.Effect<void, Error> =>
                      Effect.gen(function* () {
                        if (!agentProposal && proposalMode !== "supervised") {
                          const agentStatus = yield* agent.getStatus().pipe(
                            Effect.catch(() =>
                          ...
                          if (!hasSyncProposalTransport(agentStatus)) {
                            ...
                            isProposalBackoffActive(proposalBackoff.get(poolAddress), now)
                          ) {
                            logger.info("Agent proposal sync skipped — backoff active", {
                              pool: poolAddress,
                            });
                          } else {
                            const runTailSyncRequest = (): Effect.Effect<void, Error> =>
                              Effect.gen(function* () {
                                ...
                                if (latencySkipped) {
                                  ...
                                  const requestTailSyncProposal = (): Effect.Effect<void, Error> =>
                                    Effect.gen(function* () {
                                      const syncProposal = yield* agent
                                        .enhanceDecision(decision, {
                                          decision,
                                  ...
                                  yield* requestTailSyncProposal();
                                }
                              });
                            yield* runTailSyncRequest();
                          }
                        }
                      });
                    yield* fetchTailSyncProposal();

                    const applyTailAgentProposal = (): Effect.Effect<void, Error> =>
                      Effect.gen(function* () {
                        if (agentProposal) {
                          ...
                          const applyTailRebalanceCapitalGate = (): Effect.Effect<void, Error> =>
                            Effect.gen(function* () {
                              ...
                              const gatePosId =
                                validation.adjustedDecision?.positionId ?? decision.positionId;
                              const gatePos =
                                gatePosId !== undefined
                                  ? trackedPositions.get(gatePosId)
                                  : undefined;
                              ...
                              if (
                                ...
                                const oorGraceExpired =
                                  gatePos.oorCycleCount >= config.oorGracePeriodCycles;
                                const recoveryProb = estimateRecoveryProbability(
                                  recoveryBins,
                                  Math.abs(pool.activeBinId - positionCenter),
                                );
                          ...
                          yield* applyTailRebalanceCapitalGate();

                          if (validation.valid && validation.adjustedDecision) {
                            if (proposalMode === "suggest") {
                              const recordTailSuggestion = (
                                adjusted: AgentDecision,
                                queueProposalId: string | undefined,
                              ): Effect.Effect<void, Error> =>
                                Effect.gen(function* () {
                                  logger.info("Agent proposal suggested (advisory)", {
                                    ...
                                    pool: poolAddress,
                                    from: decision.action,
                                    suggested: adjusted.action,
                                  });
                                  yield* memory
                                    .upsert({
                                      category: "pattern",
                                      content: `Advisory suggestion for ${poolAddress}: ${adjusted.action} (confidence ${adjusted.confidence.toFixed(2)})`,
                                      poolAddress,
                                    })
                                    .pipe(Effect.catch(() => Effect.void));
                                  ...
                                  if (proposalSource === "queue" && queueProposalId) {
                                    yield* agentState
                                      .dequeueProposals([queueProposalId])
                                      .pipe(Effect.catch(() => Effect.void));
                                  }
                                });
                              yield* recordTailSuggestion(
                                validation.adjustedDecision,
                                agentProposal.proposalId,
                              );
                            } else {
                              const applyTailValidatedProposal = (
                                adjusted: AgentDecision,
                                queueProposalId: string | undefined,
                              ): Effect.Effect<void, Error> =>
                                Effect.gen(function* () {
                                  ...
                                  if (proposalSource === "queue" && queueProposalId) {
                                    appliedQueuedProposalId = queueProposalId;
                                  }
                                });
                              yield* applyTailValidatedProposal(
                                validation.adjustedDecision,
                                agentProposal.proposalId,
                              );
                            }
                            ...
                            const rejectTailAgentProposal = (
                              ...
                              Effect.gen(function* () {
                                ...
                                if (proposalSource === "queue" && queueProposalId) {
                                  yield* agentState
                                    .rejectProposal(queueProposalId)
                                    .pipe(Effect.catch(() => Effect.void));
                                }
                                yield* agentState
                                  .setAgentPolicy({ lastProposalAt: now })
                            ...
                            yield* rejectTailAgentProposal(agentProposal.proposalId);
                          }
                    ...
                    yield* applyTailAgentProposal();
                  }
                }
              });
            yield* resolveTailAgentOverlay();

            const applyTailSupervisedAndCopyBoost = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                // Supervised mode gates execution on human approval: without an applied
            ...
            yield* applyTailSupervisedAndCopyBoost();

            const applyTailHardStops = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                ...
                if (decision.action === "HOLD" && decision.poolAddress) {
                  ...
                  if (
                    ...
                    (tailPos.currentValueUsd - tailPos.depositedUsd) / tailPos.depositedUsd <
                      -0.2 &&
                ...
                const applyTailTimeStop = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    ...
                    if (
                      ...
                      if (
                        agedPos &&
                        Date.now() - agedPos.timestamp > (config.maxPositionAgeMs ?? 0)
                      ) {
                ...
                yield* applyTailTimeStop();
              });
            yield* applyTailHardStops();
            // Measured 24h price range from our own snapshots (gate 4a). Skipped
            ...
            const evaluateTailRisk = (): Effect.Effect<RiskResult, Error> =>
              Effect.gen(function* () {
                // Issue #148: the wallet safety pause is informational in shadow mode
                // (no-send by design) — it must never block a decision there.
                const pauseBlockReason = safetyPauseBlockReason(
            ...
            const riskResult: RiskResult = yield* evaluateTailRisk();

            if (!riskResult.approved) {
              const recordTailRiskRejection = (): Effect.Effect<void, Error> =>
                Effect.gen(function* () {
                  ...
                  const penalizeAppliedProposal = shouldPenalizeAppliedProposalDenial({
                    ...
                    isPreApplyRiskApproved: () =>
                      preApplyDecision !== undefined &&
                      risk.evaluate(preApplyDecision, riskCtx).approved,
                  });
                  yield* recordAppliedProposalRiskDenial(agentState, {
                    penalizeAdvisor: penalizeAppliedProposal,
              ...
              yield* recordTailRiskRejection();
              finalDecisions.push(decision);
              continue;
            }

            const recordTailSignalSnapshot = () =>
              Effect.gen(function* () {
                ...
                if (decision.action === "EXIT") {
                  ...
                  if (pendingCooldown) {
                    yield* db
                      .setPoolCooldown(pendingCooldown)
                      .pipe(Effect.catch(() => Effect.void));
                  }
                }
                ...
                const signalSnapshotId = yield* db
                  ...
                  .pipe(Effect.catch(() => Effect.succeed(null)));
                return { signalTimestamp, signalSnapshotId };
              });
            const { signalTimestamp, signalSnapshotId } = yield* recordTailSignalSnapshot();
            ...
            if (!config.paperTrading && decision.action === "ENTER") {
              ...
              const validation = yield* readTailPaperValidation();
              if (!validation.approved) {
                const recordTailPaperValidation = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    console.warn(
                ...
                yield* recordTailPaperValidation();
                finalDecisions.push(decision);
                continue;
              }
            }

            const shouldPaperExitGoLive = () =>
              config.paperTrading &&
              decision.action === "EXIT" &&
              pos?.positionPubKey &&
              config.paperModeExitLive;
            const paperExitShouldGoLive = shouldPaperExitGoLive();

            const logTailStrategyMode = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                if (decision.action === "ENTER" && config.entryStrategyType === "auto") {
                  console.info(
                    `[strategy-mode] auto resolved ${entryStrategyMode} for ${poolAddress}`,
                    {
                      volatilityStddev,
                      netDriftBins,
                    },
            ...
            if (!config.paperTrading && solFundedEntryMode && decision.action === "ENTER") {
              ...
              if (entrySizeUsd !== undefined) {
                ...
                if (!entrySolBudgetKnown || neededLamports > entrySolBudgetLamports) {
                  const recordTailCapacitySkip = (): Effect.Effect<void, Error> =>
                    Effect.gen(function* () {
                      const budgetHuman = entrySolBudgetKnown
                        ? (Number(entrySolBudgetLamports) / 1e18).toFixed(4)
                        : "unknown";
            ...
            const findTailAutonomousCandidateId = () =>
              autonomousExecution
                ...
                : undefined;
            const autonomousCandidateId = findTailAutonomousCandidateId();

            const dispatchTailExecution = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                if (paperExitShouldGoLive) {
                  ...
                  yield* paperSwapDryRun(adapter, decision, pool).pipe(
            ...
            yield* dispatchTailExecution();

            const finalizeTailOutcome = (): Effect.Effect<void, Error> =>
              Effect.gen(function* () {
                const refreshTailWalletBalance = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                ...
                yield* refreshTailWalletBalance();

                const refreshTailSolBudget = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                ...
                yield* refreshTailSolBudget();

                const recordTailAutonomousEntry = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                ...
                yield* recordTailAutonomousEntry();

                const recordTailExecutionOutcome = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    if (decision.action !== "HOLD" && !executionSkipped) {
                      const recordTailExecutionFailure = (): Effect.Effect<void, Error> =>
                        Effect.gen(function* () {
                          if (executed) {
                            cycle.poolsExecuted++;
                            recordExecutionOutcome(true);
                            ...
                            !(
                              ...
                              (isInsufficientTokenBalanceError(executionError) ||
                                ...
                                (executionError ?? "").includes(
                                  "mint simulation failed pre-broadcast",
                                ) ||
                                (executionError ?? "").includes("fell short pre-broadcast"))
                            )
                          ) {
                      ...
                      yield* recordTailExecutionFailure();
                    }
                  });
                yield* recordTailExecutionOutcome();
                const recordTailExitExecution = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    if (executed && decision.action === "EXIT") {
                      ...
                      const recordTailLossCooldown = (): Effect.Effect<void, Error> =>
                        Effect.gen(function* () {
                ...
                yield* recordTailExitExecution();
                const recordTailEntryBackoff = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    if (
                      decision.action === "ENTER" &&
                      isInsufficientTokenBalanceError(executionError)
                      ...
                      if (executedSizeUsd !== undefined) {
                        ...
                        if (redeployCandidate)
                          redeployCandidate.normalEntrySizeUsd = executedSizeUsd;
                      }
                    }
                  });
                yield* recordTailEntryBackoff();

                const finalizeTailDecision = (): Effect.Effect<void, Error> =>
                  Effect.gen(function* () {
                    ...
                    if (
                      ...
                      // SAFETY: this assertion is applied to a value whose producer and invariant are controlled by this code path.
                      const trigger =
                        decision.action.toLowerCase() as AgentRuntimeCheckin["trigger"];
                      yield* maybeSendAgentCheckin(trigger).pipe(Effect.catch(() => Effect.void));
                    }

                    finalDecisions.push(decision);
                  });
                yield* finalizeTailDecision();
              });
            yield* finalizeTailOutcome();
          }

          return finalDecisions;
        });
      return yield* runDecisionTail();
    });

  // ─── Periodic fee claiming ─────────────────────────────────────────────────

  const claimPositionRewards = (
    pos: PositionRecord,
    poolAddress: string,
    positionPubKey: string,
  ): Effect.Effect<void, never, never> =>
    Effect.gen(function* () {
      ...
      if (!config.farmRewardsEnabled) return;
      const rewardResult: ClaimRewardsResult | null = yield* adapter
  ...
  const checkHarvestGate = (
    ...
    Effect.gen(function* () {
      ...
      // real gate.
      const pending: PendingFeesResult = adapter.getPendingFees
        ? yield* adapter
            .getPendingFees(poolAddress, positionPubKey)
            .pipe(Effect.catch(() => Effect.succeed(null)))
        : null;
      if (pending) {
  ...
  const collectPositionFees = (
    poolAddress: string,
    positionPubKey: string,
    revenueConfig: RevenueConfig,
    tier: string,
  ): Effect.Effect<ClaimFeesResult | null, never, never> =>
    Effect.gen(function* () {
      const result = yield* adapter
        .claimFees(
          poolAddress,
          positionPubKey,
          revenueConfig.platformFeeRate,
          revenueConfig.revenueShareEnabled,
          revenueConfig.revenueShareOperatorPct,
          revenueConfig.feeWalletAddress,
        )
        .pipe(
          Effect.tap((r) =>
            Effect.sync(() =>
      ...
      if (!result || (result.feeX === 0 && result.feeY === 0)) return null;
      return result;
    });

  const convertCollectedFees = (
    pos: PositionRecord,
    poolAddress: string,
    claimed: ClaimFeesResult,
  ): Effect.Effect<boolean, never, never> =>
    Effect.gen(function* () {
      ...
      if (feeDestination === "compound") return false;
      const liveConversion = adapter.convertClaimedFees
        ? adapter
            .convertClaimedFees(poolAddress, feeDestination, claimed.netFeeX, claimed.netFeeY)
            .pipe(Effect.catch(() => Effect.succeed(null)))
        : Effect.succeed(null);
    ...
    Effect.gen(function* () {
      ...
      if (config.autoCompoundFees && config.paperTrading === false) {
        ...
        if (compoundGate.approved) {
          ...
          const topUp = {
            amountXAtomic: BigInt(Math.max(Math.trunc(claimed.netFeeX), 0)),
            amountYAtomic: BigInt(Math.max(Math.trunc(claimed.netFeeY), 0)),
          };
    ...
    Effect.gen(function* () {
      ...
      for (const pos of trackedPositions.values()) {
        ...
        if (pos.positionPubKey && Date.now() - pos.lastFeeClaimAt > config.feeClaimIntervalMs) {
          yield* claimPositionRewards(pos, poolAddress, pos.positionPubKey);
          if (!(yield* checkHarvestGate(poolAddress, pos.positionPubKey))) continue;

          const claimed = yield* collectPositionFees(
            poolAddress,
            pos.positionPubKey,
            revenueConfigResult,
            tier,
          ...
          const netFeesUsd = claimed.netFeesUsd ?? 0;
          ...
          yield* db
            .saveFeeClaim({
              ...
              platformFeeX: claimed.platformFeeX,
              platformFeeY: claimed.platformFeeY,
              netFeeX: claimed.netFeeX,
              netFeeY: claimed.netFeeY,
              operatorFeeX: claimed.operatorFeeX ?? 0,
              operatorFeeY: claimed.operatorFeeY ?? 0,
              txSignature: claimed.txSignature,
              feeTransferTxSignature: claimed.feeTransferTxSignature ?? null,
              reportedToApi: false,
              createdAt: Date.now(),
            })
            .pipe(Effect.catch(() => Effect.void));

          yield* reportCollectedFees(adapter, poolAddress, pos.positionPubKey, claimed, tier);
          ...
          yield* db
            .savePositionEvent({
              ...
              feesUsd: netFeesUsd,
              price: null,
              metadata: { txSignature: claimed.txSignature },
              createdAt: Date.now(),
            })
            .pipe(Effect.catch(() => Effect.void));
          ...
          if (yield* convertCollectedFees(pos, poolAddress, claimed)) continue;
          yield* compoundCollectedFees(pos, poolAddress, pos.positionPubKey, claimed, netFeesUsd);
        }
      }
    });
